...
public class LoadTimeWeaverAwareProcessor implements BeanPostProcessor, BeanFactoryAware {
	...
	/**
	 * 获取beanName=loadTimeWeaver，且class=LoadTimeWeaver.class的对象，只有{@link DefaultContextLoadTimeWeaver}，
	 * 由{@link org.springframework.context.config.LoadTimeWeaverBeanDefinitionParser}里面注册
	 */
	...
	/**
	 * 经过当前方法处理后：
	 * （1）AspectJWeavingEnabler bean中的loadTimeWeaver属性被初始化为DefaultContextLoadTimeWeaver bean
	 * （2）DefaultContextLoadTimeWeaver bean中的loadTimeWeaver属性被初始化为InstrumentationLoadTimeWeaver
	 * 题外：由{@link DefaultContextLoadTimeWeaver#setBeanClassLoader(ClassLoader)}进行初始化的
	 *
	 */
	@Override
	public Object postProcessBeforeInitialization(Object bean, String beanName) throws BeansException {
		/**
		 * spring中实现LoadTimeWeaver接口的类{@link AspectJWeavingEnabler}，
		 * 由{@link org.springframework.context.config.LoadTimeWeaverBeanDefinitionParser}里面注册
		 */
		/* 1、如果bean是LoadTimeWeaverAware实例。目前只有AspectJWeavingEnabler */
		if (bean instanceof LoadTimeWeaverAware/* 加载时间编织器感知 */) {

			LoadTimeWeaver ltw = this.loadTimeWeaver;
			if (ltw == null) {
				Assert.state(this.beanFactory != null,
						"BeanFactory required if no LoadTimeWeaver explicitly specified");
				/* 2、获取beanName=loadTimeWeaver，且class=LoadTimeWeaver.class的bean。目前只有DefaultContextLoadTimeWeaver */
				/**
				 * 获取beanName=loadTimeWeaver，且class=LoadTimeWeaver.class的对象，只有{@link DefaultContextLoadTimeWeaver}，
				 * 由{@link org.springframework.context.config.LoadTimeWeaverBeanDefinitionParser}里面注册
				 */
				// ⚠️获取beanName=loadTimeWeaver，且class=LoadTimeWeaver.class的对象 = DefaultContextLoadTimeWeaver
				ltw = this.beanFactory.getBean(
						ConfigurableApplicationContext.LOAD_TIME_WEAVER_BEAN_NAME/* loadTimeWeaver */, LoadTimeWeaver.class/* 加载时间编织器 */);
			}

			/* 3、将DefaultContextLoadTimeWeaver，设置到AspectJWeavingEnabler.loadTimeWeaver属性上 */
			// ⚠️往AspectJWeavingEnabler bean当中设置loadTimeWeaver = DefaultContextLoadTimeWeaver
