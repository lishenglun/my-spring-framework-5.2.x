...
public abstract class ApplicationObjectSupport implements ApplicationContextAware {
	...
	public final void setApplicationContext(@Nullable ApplicationContext context) throws BeansException {
		if (context == null && !isContextRequired()/* 判断是否是context必备的 */) {
			// Reset internal context state. —— 重置内部上下文状态。
			...
			// ⚠️
			// BeanNameUrlHandlerMapping和RequestMappingHandlerMapping的话，走的都是️WebApplicationObjectSupport；
	 ...
	 *
	 * 确定此应用程序对象是否需要在 ApplicationContext 中运行。
	 * <p>默认为“false”。可以重写以强制在上下文中运行
	 * （即，如果在上下文之外，则在访问器上抛出 IllegalStateException）。
	 *
	...
	protected void initApplicationContext(ApplicationContext context) throws BeansException {
		/**
		 * 1、BeanNameUrlHandlerMapping走的是AbstractDetectingUrlHandlerMapping；RequestMappingHandlerMapping走的是AbstractHandlerMapping
		 * 两者的区别在于AbstractDetectingUrlHandlerMapping extends AbstractUrlHandlerMapping，比AbstractUrlHandlerMapping多了一个detectHandlers()注册处理器方法
		 * 也就是说，BeanNameUrlHandlerMapping的setApplicationContext()方法里面不仅是注册了拦截器， 而且还是注册处理器映射的地方；
		 * 而RequestMappingHandlerMapping的setApplicationContext()只注册了拦截器
		 *
		 * 题外：BeanNameUrlHandlerMapping注册处理器映射的地方在setApplicationContext()，而RequestMappingHandlerMapping注册处理器映射的地方在afterProperties()
		 */
