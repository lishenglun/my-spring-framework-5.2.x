		...
		synchronized (this.startupShutdownMonitor) {
			...
			try {
				...
				*/
				 ...
				 *
				 * spring中的观察者模式，在传统的观察者模式的基础之上做了改动：加了一个事件驱动
				 * 传统的观察者模式，只是单纯单一的通知，相当于单一事件，不具备阶段性通知，不够灵活多变；
				 * 而加了事件驱动，相当于做了一个更细腻度的划分，广播器可以阶段性通知，然后监听器可以更加的灵活多变，根据不同的事件进行对症下药。
				 * 以及我们可以通过增加事件类型，达到灵活的拓展通知类型的效果。
				 *
				 * spring观察者模式分为这几个东西：
