...
public class GenericApplicationContext extends AbstractApplicationContext implements BeanDefinitionRegistry {
	 ...
	 * to register beans through our public methods (or the BeanFactory's). - 不执行任何操作：我们拥有一个内部BeanFactory，并依靠调用者通过我们的公共方法（或BeanFactory的方法）注册Bean。
	 * @see #registerBeanDefinition
	 */
	@Override
	...
	private static class ClassDerivedBeanDefinition extends RootBeanDefinition {
		...
		public Constructor<?>[] getPreferredConstructors() {
			...
			Constructor<?> primaryCtor = BeanUtils.findPrimaryConstructor/* 找到主构造函数 */(clazz);
			if (primaryCtor != null) {
