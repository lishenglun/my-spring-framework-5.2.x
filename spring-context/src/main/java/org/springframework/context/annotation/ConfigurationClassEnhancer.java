...
class ConfigurationClassEnhancer   {
	...
	public Class<?> enhance(Class<?> configClass, @Nullable ClassLoader classLoader) {
		if (EnhancedConfiguration.class.isAssignableFrom(configClass)) {
			/* 代表已经被代理过,因为被代理过的会实现EnhancedConfiguration接口 */
		...
		/**
		 *
		 * createClass()：得到cglib代理对象的Class对象
		 *
		 * 在一个config类的时候，先变为cglib的class，然后再变成BeanDefinition，然后在变为Bean
		 * ⚠️newEnhancer(configClass, classLoader)是创建一个cglib代理类
		 */
		Class<?> enhancedClass = createClass(newEnhancer(configClass, classLoader)/* 创建配置类的cglib代理对象 */);
	...
	private Enhancer newEnhancer(Class<?> configSuperClass, @Nullable ClassLoader classLoader) {
		...
		/**
		 * 增强一个接口，为什么要增强接口？便于判断，表示一个类已经被增强了
		 * EnhancedConfiguration extends BeanFactoryAware ，通过BeanFactoryAware的方法可以得到BeanFactory
		 * 		实现了BeanFactoryAware，那么bean在实例化之前，通过后置处理器，把BeanFactory注入到bean中
		 */
		enhancer.setInterfaces(new Class<?>[] {EnhancedConfiguration.class});
		enhancer.setUseFactory(false);
		enhancer.setNamingPolicy(SpringNamingPolicy.INSTANCE);
		/**
		 * BeanFactoryAwareGeneratorStrategy是cglib生成类的一个策略,主要为生成的cglib类中添加成员变量$$beanFactory(基于接口EnhancedConfiguration的父类BeanFactoryAware的setBeanFactory()方法)
		 */
	...
	private static class BeanMethodInterceptor implements MethodInterceptor, ConditionalCallback {
		...
		public Object intercept(Object enhancedConfigInstance, Method beanMethod, Object[] beanMethodArgs,
			...
			if (isCurrentlyInvokedFactoryMethod(beanMethod)) {
				 ...
				 * 			  }
				 * 		}
				 * 		public class CglibProxy extends AppConfig{
				 *  		// 这里是调用AppConfig的getUser()方法得到User对象
				 * 		}
				 *
				 */
		...
		private Object resolveBeanReference(Method beanMethod, Object[] beanMethodArgs,
			...
			try {
				...
				/**
				 * ⚠️：「beanFactory.getBean(beanName))」获取bean
				 */
