...
public abstract class AbstractBeanFactoryPointcutAdvisor extends AbstractPointcutAdvisor implements BeanFactoryAware {
	...
	public Advice getAdvice() {
		...
		/**
		 * 1、adviceBeanName是<aop:advisor advice-ref="txAdvice" pointcut-ref="txPoint"/>中advice-ref的属性值
		 *
		 * 2、⚠️如果advice-ref属性值引用的是事务，那么TransactionInterceptor将会在这里进行实例化
		 */
		// 若adviceBeanName对应的bd是单例的，就直接从工厂获取，没有获取到就创建（题外：有可能返回null）
