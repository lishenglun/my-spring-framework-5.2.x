...
public class AdvisedSupport extends ProxyConfig implements Advised {
	 ...
	 */
	// "能够增强当前bean的Advisor"（包含了"能够增强当前bean的Advisor"与"注册的通用的Advisor"，都作为"能够增强当前bean的Advisor"）
	 ...
	 */
	@Override
	public void setTargetSource(@Nullable TargetSource targetSource/* SingletonTargetSource */) {
		this.targetSource = (targetSource != null ? targetSource : EMPTY_TARGET_SOURCE/* EmptyTargetSource */);
	}
	...
	public Class<?> getTargetClass() {
		// targetSource：要代理的目标源，里面包含了目标对象。一般为：SingletonTargetSource。
	 ...
	 * 添加一个新的代理接口
	 *
	 * Add a new proxied interface. —— 添加一个新的代理接口。
	 *
	 ...
	 * 判断是否实现了intf接口
	 *
	 * @param intf 		SpringProxy
	 */
	@Override
	public boolean isInterfaceProxied(Class<?> intf) {
		for (Class<?> proxyIntf/* 接口 */ : this.interfaces) {
	...
	public void addAdvisors(Collection<Advisor> advisors) {
		...
		if (!CollectionUtils.isEmpty(advisors)) {
			...
			for (Advisor advisor : advisors) {
				...
				// 添加Advisor（包含了"能够增强当前bean的Advisor"与"注册的通用的Advisor"）
	...
	public List<Object> getInterceptorsAndDynamicInterceptionAdvice(Method method, @Nullable Class<?> targetClass) {
		/* 1、创建一个当前方法的拦截器链缓存Key */
		// 题外：里面展示了【如果想要实现hashMap存入一个对象Key，后续又能通过"内部数据相同，但对象不同的对象key"获取到同一个数据值】该这么做，只需要重写hashCode()、equals()即可
		...
		/* 4、如果缓存中没有，则"从能够增强当前bean的Advisor中，获取能够增强当前方法的Advisor，然后从这些Advisor当作，获取其Advice对应的能够拦截当前方法的拦截器链"，获取到之后放入缓存中 */
		if (cached == null) {
			// ⚠️从能够增强当前bean的Advisor中，获取能够增强当前方法的Advisor，然后从这些Advisor当作，获取其Advice对应的能够拦截当前方法的拦截器链（也就是Advisor当中Advice所对应的所有MethodInterceptor）
	...
	private static final class MethodCacheKey implements Comparable<MethodCacheKey> {
		...
		题外：如果想要实现hashMap存入一个对象Key，后续又能通过"内部数据相同，但对象不同的对象key"获取到同一个数据值，只需要重写hashCode()、equals()即可

		*/

		/**
		 * 从hashMap中根据MethodCacheKey获取数据，获取到数据后，"用于获取数据的key"与"获取到数据的key"作比较会调用此方法。
		 * 由"用于获取数据的key"调用equals()，入参是"获取到数据的key"。
		 *
		 * @param other		获取到数据的key
		 */
		@Override
		public boolean equals(@Nullable Object other) {
			// 【两个key相等 || 里面的method相等】即可
			return (this == other || (other instanceof MethodCacheKey &&
					this.method == ((MethodCacheKey) other).method));
		}

		/**
		 * 存入数据和获取数据用的，以及比对数据用的(从hashmap中获取到了数据，会先比对hashCode，然后再调用equals进行比较)
		 */
		@Override
		public int hashCode() {
