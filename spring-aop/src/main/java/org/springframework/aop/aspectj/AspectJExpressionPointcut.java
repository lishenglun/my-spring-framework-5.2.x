...
public class AspectJExpressionPointcut extends AbstractExpressionPointcut
	...
	// 从此处可以看出，Spring支持的AspectJ的切入点表达式一共有10种（加上后面的自己的Bean方式一共11种）
	...
	//
	@Nullable
	private Class<?> pointcutDeclarationScope/* 切入点声明范围 */;

	private String[] pointcutParameterNames/* 切入点参数名称 */ = new String[0];

	private Class<?>[] pointcutParameterTypes/* 切入点参数类型 */ = new Class<?>[0];
	...
	/**
	 * 1、题外：PointcutExpression是AspectJ框架中的接口
	 *
	 * 2、题外：它最终通过一系列操作，由org.aspectj.weaver.tools.PointcutParser#parsePointcutExpression()从字符串表达式解析出来
	 */
	// 这个对象是指"底层AspectJ框架的切入点表达式对象"（提示：不是Spring的切入点表达式对象）
	// PointcutExpression = PointcutExpressionImpl对象，里面保存了切入点表达式
	@Nullable
	...
	/**
	 * 1、由AspectJAwareAdvisorAutoProxyCreator#postProcessAfterInitialization()
	 * ——> AbstractAutoProxyCreator#wrapIfNecessary()
	 * ——> getAdvicesAndAdvisorsForBean()
	 * ——> findEligibleAdvisors()
	 * ——> AopUtils.findAdvisorsThatCanApply()
	 * ——> canApply()方法粒度匹配
	 * ——> canApply()方法粒度匹配，里面调用当前方法获取ClassFilter，用于先匹配类是否符合规则
	 *
	 * 2、ClassFilter：用于匹配类是否符合规则
	 */
	@Override
	...
	/**
	 * 获取底层AspectJ框架的切入点表达式对象(没有的话会构建)，PointcutExpression PointcutExpressionImpl，里面保存了切入点表达式
	 *
	 * 1、题外：相当于是把Spring的切入点表达式对象，转换为AspectJ框架的切入点表达式对象
	 *
	 * 2、题外：该方法，最开始由：
	 ...
	 * ——> canApply()方法粒度匹配
	 * ——> canApply()方法粒度匹配
	 * ——> pc.getClassFilter()先匹配这个类是不是符合规则 = {@link AspectJExpressionPointcut#getClassFilter()} 里面调用
	 *
	 * Check whether this pointcut is ready to match,
	 * lazily building the underlying AspectJ pointcut expression.
	 *
	 * 检查这个切入点是否准备好匹配，懒惰地构建底层的AspectJ切入点表达式。
	 */
	private PointcutExpression obtainPointcutExpression() {
		...
		if (this.pointcutExpression == null) {
			...
			// ⚠️构建底层AspectJ框架的切入点表达式对象（提示：不是Spring中的切入点表达式对象）
			// 最终是返回一个PointcutExpression = PointcutExpressionImpl对象，里面保存了切入点表达式
			this.pointcutExpression = buildPointcutExpression(this.pointcutClassLoader);
		}
		return this.pointcutExpression/* PointcutExpressionImpl */;
	}
	 ...
	 * 最终是返回一个PointcutExpression = PointcutExpressionImpl对象，里面保存了切入点表达式
	 *
	 * Build the underlying AspectJ pointcut expression. —— 构建底层AspectJ切入点表达式。
	 */
	private PointcutExpression buildPointcutExpression(@Nullable ClassLoader classLoader) {
		// 创建PointcutParser(切入点解析器)
		PointcutParser parser = initializePointcutParser/* 初始化切入点解析器 */(classLoader);

		PointcutParameter[] pointcutParameters = new PointcutParameter[this.pointcutParameterNames.length];
		// 遍历切入点参数名称
		for (int i = 0; i < pointcutParameters.length; i++) {
			// 通过PointcutParser，创建PointcutParameterImpl对象，里面保存"切入点参数名称"和"切入点参数类型"
			pointcutParameters[i] = parser.createPointcutParameter(
					this.pointcutParameterNames[i], this.pointcutParameterTypes[i]);
		}
		/**
		 * 1：resolveExpression()：获取切入点表达式，里面只是判空了一下
		 *
		 * 2、replaceBooleanOperators()：替换切入点表达式当中的布尔运算符：and替换为&&，or替换为||，not替换为!
		 * 所以其实，这个函数可以忽略，最终得到的还是一个切入点表达式
		 */
		// 最终是返回一个PointcutExpression = PointcutExpressionImpl对象，里面保存了切入点表达式
		return parser.parsePointcutExpression/* 解析切入点表达式 */(replaceBooleanOperators(resolveExpression()),
	...
	/**
	 * Initialize the underlying AspectJ pointcut parser. —— 初始化底层AspectJ切入点解析器
	 */
	private PointcutParser initializePointcutParser(@Nullable ClassLoader classLoader) {
		// 创建PointcutParser对象返回，
		// 并往里面保存classLoader
		PointcutParser parser = PointcutParser
				// 获取支持，指定原语的切入点解析器，并使用指定的类加载器进行解析
				.getPointcutParserSupportingSpecifiedPrimitivesAndUsingSpecifiedClassLoaderForResolution(
						SUPPORTED_PRIMITIVES/* HashSet<PointcutPrimitive> */, classLoader);

		// 保存BeanPointcutDesignatorHandler
		parser.registerPointcutDesignatorHandler/* 注册切入点指示符处理程序 */(new BeanPointcutDesignatorHandler()/* Bean切入点指示符处理程序 */);
		return parser;
	}


	/**
	 * 替换切入点表达式当中的布尔运算符：and替换为&&，or替换为||，not替换为!
	 *
	 ...
	 */
	private String replaceBooleanOperators/* 替换布尔运算符 */(String pcExpr) {
		// (1)替换切入点表达式当中的" and "为" && "
	...
	/**
	 * 这是ClassFilter匹配类，借助PointcutExpression#couldMatchJointsInType()去匹配
	 */
	@Override
	 ...
	 * @param hasIntroductions 			是否存在能够增强当前bean的引介增强Advisor
	 * @return
	 */
	@Override
	public boolean matches(Method method, Class<?> targetClass, boolean hasIntroductions/* 引介增强 */) {
		// 构建底层AspectJ框架的切入点表达式对象
		// 题外：因为这里不获取，所以是构建！
		...
		// 上面翻译：在Spring中对this、target、@this、@target、@annotation进行特殊处理——我们可以优化，因为我们知道我们完全有这个类，并且在运行时永远不会有匹配的子类。

		if (shadowMatch.alwaysMatches/* 总是匹配 */()) {
			return true;
		}
		else if (shadowMatch.neverMatches/* 从不匹配 */()) {
			return false;
		}
		else {
			// the maybe case —— 可能的情况
			if (hasIntroductions) {
				return true;
			}
			...
			// runtime subtype.
			// 上面翻译：可能会返回匹配测试 - 如果测试中涉及任何子类型敏感变量（this、target、at_this、at_target、at_annotation），那么我们说这不是匹配，因为在 Spring 中永远不会有不同的运行时子类型。
	...
	public boolean isRuntime() {
		return obtainPointcutExpression()/* PointcutExpressionImpl */.mayNeedDynamicTest();
	}
	...
	private ShadowMatch getTargetShadowMatch(Method method, Class<?> targetClass) {
		...
		if (targetMethod.getDeclaringClass().isInterface()) {
			...
			// 上面翻译：尝试为子接口匹配考虑的继承方法构建最具体的接口，尤其是代理类。注意：AspectJ只会考虑Method.getDeclaringClass()。
			...
			if (ifcs.size() > 1) {
				try {
					...
					// 上面翻译：已实现的接口可能会暴露冲突的方法签名...继续使用原始目标方法。
				}
			}
		}
