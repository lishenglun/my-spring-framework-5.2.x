...
final class InstantiationModelAwarePointcutAdvisorImpl
	...
	/**
	 * 创建一个Advisor，保存切入点信息；同时根据Advice方法上的Advice注解类型，创建对应的Advice进行保存。Advice里面也保存了切入点信息
	 *
	 * @param declaredPointcut				切点信息
	 * @param aspectJAdviceMethod			Advice方法(切面方法)
	...
	public InstantiationModelAwarePointcutAdvisorImpl(AspectJExpressionPointcut declaredPointcut,
		...
		if (aspectInstanceFactory.getAspectMetadata().isLazilyInstantiated()) {
			...
			// 上面翻译：使其动态化：必须从实例化前状态转变为实例化后状态。如果不是动态切入点，可能会在第一次评估后被Spring AOP基础设施优化掉。
			...
			// A singleton aspect. —— 单例切面

			this.pointcut = this.declaredPointcut;
			this.lazy = false;
			// ⚠️根据Advice方法上的Advice注解类型，创建对应的Advice返回
