...
public class UrlPathHelper {
	...
	public String getLookupPathForRequest(HttpServletRequest request) {
		// Always use full path within current servlet context? —— 总是在当前 servlet 上下文中使用完整路径？
		if (this.alwaysUseFullPath) {
			return getPathWithinApplication/* 获取应用程序内的路径 */(request);
		}
		/**
		 * 例如：
		 * <servlet-mapping>
		 * 		<servlet-name>mvc-test2</servlet-name>
		 * 		<url-pattern>/</url-pattern>
		 * 	</servlet-mapping>
		 */
		// Else, use path within current servlet mapping if applicable —— 否则，使用当前 servlet 映射中的路径（如果适用）
		String rest = getPathWithinServletMapping/* 获取Servlet映射中的路径 */(request);
		if (!"".equals(rest)) {
			return rest;
		}
		else {
			// ⚠️获取servlet的路径（uri中去掉，项目访问路径）
	 ...
	 * @see #getLookupPathForRequest
	 */
	public String getPathWithinApplication/* 获取应用程序内的路径 */(HttpServletRequest request) {
		// 获取上下文路径，也就是项目的访问路径
		// 例如：项目的访问路径是：/springmvc，那么http请求必须是：http://127.0.0.1:8080/springmvc
		String contextPath = getContextPath(request);
		// 获取请求的uri，包含了项目的访问路径
		// 例如：http://127.0.0.1:8080/springmvc/userlist
		String requestUri = getRequestUri(request);
		// 获取servlet路径 - 去掉uri中，项目的访问路径，得到servlet路径
		// 例如：/userlist
