...
public class ServletRequestAttributes extends AbstractRequestAttributes {
	 ...
	 * @param scope the scope identifier	作用域标识
	 * @return
	 */
	@Override
	public Object getAttribute(String name, int scope) {
		// 1、如果作用域是request作用域，就从request作用域中获取属性值
		...
		// 2、如果作用域是session作用域，就从session作用域中获取属性值
		else {
	...
	public void setAttribute(String name, Object value, int scope) {
		// 1、如果是request作用域，就往request作用域中设置属性名称和属性值
		if (scope == SCOPE_REQUEST) {
			if (!isRequestActive()) {
				throw new IllegalStateException(
		...
		// 2、如果是session作用域，就往session作用域中设置属性名称和属性值
		else {
			// 获取HttpSession
