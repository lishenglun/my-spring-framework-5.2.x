...
public abstract class DataSourceUtils {
	...
	public static Connection doGetConnection(DataSource dataSource) throws SQLException {
		...
		/* 1、从本地线程变量中，获取当前线程中已经存在的连接，保证线程中的数据库操作都是使用同一个事务连接 */
		...
		if (conHolder != null && (conHolder.hasConnection() || conHolder.isSynchronizedWithTransaction())) {
			...
			// 获取当前线程中已经存在的连接，保证线程中的数据库操作都是使用同一个事务连接
			return conHolder.getConnection();
		}

		/* 2、当前线程不存在连接，则获取新的连接 */
		// Else we either got no holder or an empty thread-bound holder here. —— 否则，我们要么没有持有器，要么这里有一个空的线程绑定持有器。

		logger.debug("Fetching JDBC Connection from DataSource");
		// 获取新的连接
		Connection con = fetchConnection(dataSource);

		/* 3、如果当前线程支持同步，则线程绑定连接，方便事务中后面的jdbc操作使用相同的连接 */
		// 判断当前线程是否支持同步
		if (TransactionSynchronizationManager.isSynchronizationActive()) {
			/* 当前线程支持同步 */

			try {
				// Use same Connection for further JDBC actions within the transaction. —— 对事务中的进一步JDBC操作使用相同的连接。
				// Thread-bound object will get removed by synchronization at transaction completion. —— 线程绑定对象，将在事务完成时通过同步删除。
				...
				if (holderToUse != conHolder) {
					// 往本地线程变量中，存入数据源和对应的连接持有器
					TransactionSynchronizationManager.bindResource(dataSource, holderToUse);
				}
			}
			catch (RuntimeException ex) {
				// Unexpected exception from external delegation call -> close Connection and rethrow. —— 来自外部委托调用的意外异常 -> 关闭连接并重新抛出。
	...
	public static Integer prepareConnectionForTransaction(Connection con, @Nullable TransactionDefinition definition)
		...
		/* 1、设置数据库连接的只读标识 */
		// Set read-only flag.
		if (definition != null && definition.isReadOnly()/* 判断当前事务是不是只读 */) {
			try {
				if (debugEnabled) {
		...
		if (definition != null && definition.getIsolationLevel() != TransactionDefinition.ISOLATION_DEFAULT/* -1 */) {
			if (debugEnabled) {
	 ...
	 * <p>Directly accessed by {@link TransactionAwareDataSourceProxy}.
	 *
	 * 实际上关闭从给定数据源获得的给定连接。与{@link releaseConnection}相同，但抛出原始SQLException。<p>由{@link TransactionAwareDataSourceProxy}直接访问。
	 *
	 * @param con the Connection to close if necessary
	 * (if this is {@code null}, the call will be ignored)
	 * @param dataSource the DataSource that the Connection was obtained from
	...
	public static void doReleaseConnection(@Nullable Connection con, @Nullable DataSource dataSource) throws SQLException {
		...
		if (dataSource != null) {
			// 当前线程存在事务的情况下，说明存在共用数据库连接，直接使用ConnectionHolder中的released()方法进行连接数减1，而不是真正的释放连接
			ConnectionHolder conHolder = (ConnectionHolder) TransactionSynchronizationManager.getResource(dataSource);

			if (conHolder != null && connectionEquals(conHolder, con)) {
				// It's the transactional Connection: Don't close it. —— 这是事务连接：不要关闭它。
				// 连接数-1，以及把当前连接置为空
				conHolder.released();
				return;
			}
