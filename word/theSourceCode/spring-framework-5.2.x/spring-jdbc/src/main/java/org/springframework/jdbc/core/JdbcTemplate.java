...
public class JdbcTemplate extends JdbcAccessor implements JdbcOperations {
	...
	/**
	 * 题外：与{@link #execute(PreparedStatementCreator, PreparedStatementCallback)}并无太大差别，都是做一些常规的处理，诸如获取连接、释连接等，
	 * 但是，有一个地方是不一样的，就是statement的创建。这里直接使用connection创建，而它由于是处理带有参数的SQL，所以使用的是PreparedStatementCreator类来创建的。
	 * 一个是普通的Statement， 另一个是PreparedStatement
	 *
	...
	/**
	 * 题外：与带有参数进行sql查询的{@link #query(PreparedStatementCreator, PreparedStatementSetter, ResultSetExtractor)}方法的不同，是少了参数及参数类型的传递，
	 * 自然也少了PreparedStatementSetter的封装。与此同时，由于少了PreparedStatementSetter类型的传入，调用的execute()也有所改变了
	 *
	...
	public <T> T queryForObject(String sql, Class<T> requiredType) throws DataAccessException {
		/**
		 * 1、getSingleColumnRowMapper(requiredType)：SingleColumnRowMapper
		 *
		 * 查看{@link SingleColumnRowMapper#mapRow(ResultSet, int)}
		 */
	...
	//-------------------------------------------------------------------------
	// Methods dealing with prepared statements —— 处理准备好的语句的方法
	//-------------------------------------------------------------------------

	/**
	 * execute()方法是最基础的操作，其它的操作比如update、query等方法，则是传人不同的PreparedStatementCallback参数，来执行不同的逻辑。
	 * execute()作为数据库操作的核心人口，将大多数数据库操作相同的步骤统一封装，而将个性化的操作使用参数PreparedStatementCallback进行回调。
	 */
	@Override
	@Nullable
	public <T> T execute(PreparedStatementCreator psc, PreparedStatementCallback<T> action)
		...
		// 获取数据库连接
		// 题外：在数据库连接方面，Spring主要考虑的是关于事务方面的处理。基于事务处理的特殊性，Spring需要保证线程中的数据库操作都是使用同一个事务连接。
		Connection con = DataSourceUtils.getConnection(obtainDataSource()/* 获取数据源 */);
		...
		try {
			...
			/**
			 * 1、处理一些通用方法外的个性化处理,也就是PreparedStatementCallback类型的参数的doInPreparedStatement()方法的回调。
			 */
			// 调用回调函数
			...
			// Release Connection early, to avoid potential connection pool deadlock
			// in the case when the exception translator hasn't been initialized yet.
			// 上面翻译：尽早释放连接，以避免在异常转换器尚未初始化的情况下潜在的连接池死锁。
		...
		return execute(psc, new PreparedStatementCallback<T>() {
			...
			public T doInPreparedStatement(PreparedStatement ps) throws SQLException {
				...
				try {
					 ...
					 * 另外在构造RowMapperResultSetExtractor的时候我们又将自定义的rowMapper设置了进去
					 */
					// 负责将结果进行封装并转换至POJO
					return rse.extractData(rs);
				}
	...
	public <T> T query(String sql, Object[] args, int[] argTypes, ResultSetExtractor<T> rse) throws DataAccessException {
		/**
		 * 1、newArgTypePreparedStatementSetter(args, argTypes)
		 * 与update()中都同样使用了newArgTypePreparedstatementsetter
		 */
	 ...
	 *
	 * @param psc
	 * @param pss						ArgPreparedStatementSetter
	 * @return
	 * @throws DataAccessException
	 */
	protected int update(final PreparedStatementCreator psc, @Nullable final PreparedStatementSetter pss)
		...
		return updateCount(/* ⚠️ */execute(psc, ps -> {
			try {
				if (pss != null) {
					// 设置PrepareStatement所需要的全部参数
					// 题外：pss = ArgumentPreparedStatementSetter
					pss.setValues(ps);
				}
	...
	public int update(String sql, @Nullable PreparedStatementSetter pss) throws DataAccessException {
		/**
		 * 1、new SimplePreparedStatementCreator(sql)
	...
	public int update(String sql, @Nullable Object... args) throws DataAccessException {
		/**
		 * 1、newArgPreparedStatementSetter(args)
		 * 使用ArgumentPreparedStatementSetter对参数与参数类型进行封装
		 */
	...
	protected void applyStatementSettings(Statement stmt) throws SQLException {
		...
		if (fetchSize != -1) {
			...
			// setFetchSize()的意思是当调用rs.next时，ResultSet会一次性从服务器上取得多少行数据回来，
