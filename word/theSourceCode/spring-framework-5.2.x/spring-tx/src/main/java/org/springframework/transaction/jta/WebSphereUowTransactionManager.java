...
public class WebSphereUowTransactionManager extends JtaTransactionManager
	...
	public <T> T execute(@Nullable TransactionDefinition definition, TransactionCallback<T> callback)
		...
		// 挂起的资源
		SuspendedResourcesHolder suspendedResources = (!joinTx ? suspend(null)/* 挂起 */ : null);
		...
		try {
			...
			action = new UOWActionAdapter<>(def, callback/* ⚠️ */, actualTransaction, !joinTx, newSynch, debug);

			/**
			 * {@link UOWActionAdapter#run()}
			 */
			// ⚠️内部调用执行action#run()，从而调用执行callback
