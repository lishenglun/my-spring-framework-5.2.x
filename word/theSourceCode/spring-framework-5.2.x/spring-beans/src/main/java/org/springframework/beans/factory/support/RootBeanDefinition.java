...
public class RootBeanDefinition extends AbstractBeanDefinition {
	...
	boolean allowCaching/* 允许缓存 */ = true;
	...
	/** Package-visible field that marks the constructor arguments as resolved. —— 将构造函数的参数标记为已解析的包装可见字段。 */
	...
	// @Resource标注的字段和方法，@Autowired标注的的字段和方法
	@Nullable
	private Set<Member> externallyManagedConfigMembers;

	// 初始化方法名称集合（@PostConstruct标注的方法名称。注意：xml<bean init-method="">标注的方法不会加入到这里面）
	// 注意：里面存储的是初始化的方法名称，方法名称！
	// 注意：@PostConstruct标注的方法可以有多个！
	@Nullable
	private Set<String> externallyManagedInitMethods;

	// 销毁方法名称集合（@PreDestroy标注的方法名称。注意：xml<bean destroy-method="">标注的方法不会加入到这里面）
	// 注意：里面存储的是销毁化的方法名称，方法名称！
	// 注意：@PreDestroy标注的方法可以有多个！
	@Nullable
	...
	public void registerExternallyManagedInitMethod(String initMethod) {
		synchronized (this.postProcessingLock) {
			...
			this.externallyManagedInitMethods/* 外部管理的初始化方法 */.add(initMethod);
		}
	}
	...
	public void registerExternallyManagedDestroyMethod(String destroyMethod) {
		synchronized (this.postProcessingLock) {
			...
			this.externallyManagedDestroyMethods/* 外部管理的销毁方法 */.add(destroyMethod);
		}
	}
