...
public class CustomEditorConfigurer implements BeanFactoryPostProcessor, Ordered {
	...
	/**
	 * ⚠️⚠️⚠️在{@link CustomEditorConfigurer#postProcessBeanFactory(ConfigurableListableBeanFactory)}}中，
	 * 会将"属性编辑器注册器集合"中的数据{@link propertyEditorRegistrars}放入到BeanFactory的"属性编辑器注册器集合"中{@link BeanFactory.propertyEditorRegistrars中}
	 * 和将"自定义属性编辑器集合"中的数据{@link customEditors}放入到BeanFactory的"自定义属性编辑器集合"中{@link BeanFactory.customEditors}
	 */
	...
	@Override
	public void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory/* beanFactory = DefaultListableBeanFactory */) throws BeansException {
		/* 1、把当前对象中的"属性编辑器注册器"全部添加到BeanFactory当中 */
		// 如果this.propertyEditorRegistrars中有值，就把里面的值，通过调用beanFactory.addPropertyEditorRegistrar()，给加入到beanFactory.propertyEditorRegistrars里面去
		if (this.propertyEditorRegistrars != null) {
			for (PropertyEditorRegistrar propertyEditorRegistrar : this.propertyEditorRegistrars) {
				beanFactory.addPropertyEditorRegistrar(propertyEditorRegistrar);
			}
		}
		/* 2、把当前对象中的"属性编辑器"全部添加到BeanFactory当中 */
		// 如果this.customEditors中有值，就把里面的值，通过调用beanFactory#registerCustomEditor()，给加入到beanFactory.customEditors里面去
