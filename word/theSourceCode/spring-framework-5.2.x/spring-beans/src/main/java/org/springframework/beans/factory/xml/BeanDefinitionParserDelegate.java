 ...
 * {@link BeanDefinitionParser(解析器) BeanDefinitionParsers}或 {@link BeanDefinitionDecorator(装饰器) BeanDefinitionDecorators} 使用。
 * <p>
 ...
 * @since 2.0
 */
public class BeanDefinitionParserDelegate/* bd解析器委托 */ {
	...
	protected void populateDefaults(DocumentDefaultsDefinition defaults, @Nullable DocumentDefaultsDefinition parentDefaults, Element root) {
		...
		if (root.hasAttribute(DEFAULT_AUTOWIRE_CANDIDATES_ATTRIBUTE/* default-autowire-candidates */)) {
			defaults.setAutowireCandidates(root.getAttribute(DEFAULT_AUTOWIRE_CANDIDATES_ATTRIBUTE/* default-autowire-candidates */));
		} else if (parentDefaults != null) {
			defaults.setAutowireCandidates(parentDefaults.getAutowireCandidates());
		}

		if (root.hasAttribute(DEFAULT_INIT_METHOD_ATTRIBUTE/* default-init-method */)) {
			defaults.setInitMethod(root.getAttribute(DEFAULT_INIT_METHOD_ATTRIBUTE/* default-init-method */));
		} else if (parentDefaults != null) {
			defaults.setInitMethod(parentDefaults.getInitMethod());
		}

		if (root.hasAttribute(DEFAULT_DESTROY_METHOD_ATTRIBUTE/* default-destroy-method */)) {
			defaults.setDestroyMethod(root.getAttribute(DEFAULT_DESTROY_METHOD_ATTRIBUTE/* default-destroy-method */));
		} else if (parentDefaults != null) {
			defaults.setDestroyMethod(parentDefaults.getDestroyMethod());
		}
		// 【this.readerContext.extractSource(root)】默认返回的是null
		defaults.setSource(this.readerContext.extractSource(root)/* 提取来源 */);
	}
	 ...
	 */
	@Nullable
	public String[] getAutowireCandidatePatterns/* 获取Autowire候选模式 */() {
		String candidatePattern = this.defaults.getAutowireCandidates();
		return (candidatePattern != null ? StringUtils.commaDelimitedListToStringArray/* 逗号分隔列表到字符串数组 */(candidatePattern) : null);
	}
	...
	public BeanDefinitionHolder parseBeanDefinitionElement(Element ele, @Nullable BeanDefinition containingBean) {
		...
		// ⚠️对bean元素进行详细的解析，解析完成后就得到一个：完整的解析完xml文件属性值的BeanDefinition，解析工作就完成了！
		...
		if (beanDefinition != null) {
			...
			// 返回一个BeanDefinitionHolder，封装了beanDefinition、beanName、aliasesArray
	...
	public AbstractBeanDefinition parseBeanDefinitionElement(
		...
		try {
			// 创建在bean信息的AbstractBeanDefinition对象，实际实现是GenericBeanDefinition
	...
	public Object parsePropertyValue(Element ele, BeanDefinition bd, @Nullable String propertyName) {
		...
		/**
		 * 处理集合标签的情况，所以获取子元素
		 * 	<bean id="a" class="com.springstudy.mashibing.s_02.bean.A">
		...
		if (hasRefAttribute/* ref */) {
			...
			return ref;
		} else if (hasValueAttribute/* value */) {
	...
	public Object parsePropertySubElement(Element ele, @Nullable BeanDefinition bd, @Nullable String defaultValueType) {
		if (!isDefaultNamespace(ele)) {
			...
			String refName = ele.getAttribute(BEAN_REF_ATTRIBUTE/* bean */);
			boolean toParent = false;
			if (!StringUtils.hasLength(refName)) {
				// A reference to the id of another bean in a parent context.
				refName = ele.getAttribute(PARENT_REF_ATTRIBUTE/* parent */);
				toParent = true;
				if (!StringUtils.hasLength(refName)) {
	 ...
	 *
	 * @param ele          the element to parse
	 * @param containingBd the containing bean definition (if any) —— 包含的bd（如果有）
	 * @return the resulting bean definition
	 */
	@Nullable
	public BeanDefinition parseCustomElement(Element ele, @Nullable BeanDefinition containingBd) {
		// 获取标签对应的"命名空间uri"
		String namespaceUri/* 命名空间uri */ = getNamespaceURI(ele);
		...
		// 题外：在第一次实例化对应的"命名空间处理器"时，会调用"命名空间处理器的init()方法"，初始化和注册标签对应的解析器
		// 题外：注册的标签解析器是放入到NamespaceHandlerSupport.parses()里面
		NamespaceHandler/* 命名空间处理器 */ handler = this.readerContext.getNamespaceHandlerResolver()/* DefaultNamespaceHandlerResolver */
				.resolve(namespaceUri);
		if (handler == null) {
			error("Unable to locate Spring NamespaceHandler for XML schema namespace [" + namespaceUri + "]", ele);
			return null;
		}
		// ⚠️调用"命名空间处理器"进行解析，里面会通过标签名称获取到对应的标签解析器，然后用标签解析器进行解析处理
	...
	/**
	 * Determine whether the given URI indicates the default namespace. —— 确定给定的URI是否指示默认命名空间。
	 */
	public boolean isDefaultNamespace(@Nullable String namespaceUri) {
		// 判断namespaceUri是不是等于【http://www.springframework.org/schema/beans】
	...
	public boolean isDefaultNamespace(Node node) {
		return isDefaultNamespace(getNamespaceURI(node)/* 获取命名空间url */);
	}
