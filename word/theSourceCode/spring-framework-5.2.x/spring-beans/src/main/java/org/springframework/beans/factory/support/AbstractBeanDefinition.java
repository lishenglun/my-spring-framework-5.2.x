...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	/**
	 * ⚠️<bean autowire="no"/>标签中的autowire属性值！
	 *
	...
	/**
	 * ⚠️<bean autowire="byName"/>标签中的autowire属性值！
	 *
	...
	/**
	 * ⚠️<bean autowire="byType"/>标签中的autowire属性值！
	 *
	...
	/**
	 * ⚠️<bean autowire="constructor"/>标签中的autowire属性值！
	 *
	 ...
	 * 但是该bean本身还是可以使用自动装配来注入其它bean的
	 */
	private boolean autowireCandidate = true;

	/**
	 * 自动装配时，当出现多个bean候选者时，将作为首选者
	 */
	...
	/**
	 * 实例供应器：用于在createBean()创建bean实例对象的
	 *
	 * 要想当前bd中这个属性不为空，可以采用BFPP的方式修改bd，注入该属性！
	 */
	@Nullable
	 ...
	 *
	 *
	 * 题外：methodOverrides里面的数组哪里来的？解析xml配置文件时创建bd创建的！如果加了<lookup-method>标签，那么里面就有值
	 */
	...
	/**
	 * 是否是应用程序本身定义的，而不是用户定义的，创建AOP的时候为true
	 */
	...
	/**
	 * Return the current bean class name of this bean definition. —— 返回此 bean 定义的当前 bean 类名。
	 */
	@Override
	@Nullable
	...
	public boolean hasConstructorArgumentValues() {
		// 不为null，并且不等于空，则代表存在构造参数，返回true！
		/**
		 * constructorArgumentValues里面的值是
		 * 	<bean id="person" class="com.springstudy.mashibing.s_16.Person" scope="prototype">
		 * 		<constructor-arg name="id" value="1"/>
		 * 		<constructor-arg name="name" value="zhangsan"/>
		 * 	</bean>
		 * 	里面<constructor-arg name="id" value="1"/> 标签的值
		 */
	...
	/**
	 * Validate this bean definition. —— 验证此 bean 定义。
	 *
	...
	public void prepareMethodOverrides() throws BeanDefinitionValidationException {
		// Check that lookup methods exist and determine their overloaded status. —— 检查查找方法是否存在并确定它们的重载状态。
	...
	protected void prepareMethodOverride(MethodOverride mo) throws BeanDefinitionValidationException {
		...
		if (count == 0) {
			...
			// 标记methodOverride暂未被覆盖，避免参数类型检查的开销，不需要对它进行任何相关的检查工作了，后面我在进行实例化的时候，就可以直接对它进行调用了
