...
public class RequestParamMethodArgumentResolver extends AbstractNamedValueMethodArgumentResolver
	...
	// 是否使用默认的处理器
	private final boolean useDefaultResolution/* 使用默认分辨率 */;
	...
	protected Object resolveName(String name, MethodParameter parameter, NativeWebRequest request) throws Exception {
		// 把当前的request对象，转换成HttpServletRequest对象
		...
		if (arg == null) {
			// ⚠️根据方法参数名称，去request里面对应的参数值
			// 题外：先获取方法参数名称，然后根据方法参数名称，去request里面对应的参数值
	...
	protected void handleMissingValue(String name, MethodParameter parameter, NativeWebRequest request)
		...
		if (MultipartResolutionDelegate.isMultipartArgument(parameter)) {
			if (servletRequest == null || !MultipartResolutionDelegate.isMultipartRequest(servletRequest)) {
				throw new MultipartException("Current request is not a multipart request"/* 当前请求不是上传请求 */);
			}
			else {
