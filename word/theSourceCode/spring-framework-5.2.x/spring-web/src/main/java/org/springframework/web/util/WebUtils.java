...
public abstract class WebUtils {
	 ...
	 *
	 * 判断给定的请求，是否是包含请求，即不是从外部进来的顶级HTTP请求。
	 * <p>检查“javax.servlet.include.request_uri”请求属性是否存在。可以检查仅存在于包含请求中的任何请求属性。
	 *
	 ...
	 */
	public static void exposeErrorRequestAttributes/* 公开错误请求属性 */(HttpServletRequest request, Throwable ex,
			@Nullable String servletName) {

		/* 1、往request域中设置错误信息 */

		/* 如果request域中不存在这个属性名对应的属性值，设置该属性名对应的属性值！ */

		// 如果request域中不存在状态码，则设置状态码
		exposeRequestAttributeIfNotPresent/* 如果不存在，则公开请求属性 */(request, ERROR_STATUS_CODE_ATTRIBUTE/* javax.servlet.error.status_code */, HttpServletResponse.SC_OK/* 200 */);
		// 如果request域中不存在异常类型，则设置当前异常类型
		exposeRequestAttributeIfNotPresent(request, ERROR_EXCEPTION_TYPE_ATTRIBUTE/* javax.servlet.error.exception_type */, ex.getClass());
		// 如果request域中不存在错误消息，则设置错误消息
		exposeRequestAttributeIfNotPresent(request, ERROR_MESSAGE_ATTRIBUTE/* javax.servlet.error.message */, ex.getMessage());
		// 如果request域中不存在异常对象，则设置异常对象
		exposeRequestAttributeIfNotPresent(request, ERROR_EXCEPTION_ATTRIBUTE/* javax.servlet.error.exception */, ex);
		// 如果request域中不存在请求uri，则设置请求uri
		exposeRequestAttributeIfNotPresent(request, ERROR_REQUEST_URI_ATTRIBUTE/* javax.servlet.error.request_uri */, request.getRequestURI());

		if (servletName != null) {
			exposeRequestAttributeIfNotPresent(request, ERROR_SERVLET_NAME_ATTRIBUTE/* javax.servlet.error.servlet_name */, servletName);
		}
	}
