...
public class DefaultSessionAttributeStore implements SessionAttributeStore {
	...
	public void storeAttribute(WebRequest request, String attributeName, Object attributeValue) {
		...
		String storeAttributeName = getAttributeNameInSession/* 在会话中获取属性名称 */(request, attributeName);
		// 往session作用域中设置属性名称和属性值
		request.setAttribute(storeAttributeName, attributeValue, WebRequest.SCOPE_SESSION/* 1：表示作用域是Session */);
	}

	/**
	 * 从session作用域获取属性值
	 *
	 * @param request       the current request				request，通过request可以获取到HttpSession
	 * @param attributeName the name of the attribute	属性名称
	 * @return
	 */
	@Override
	@Nullable
	public Object retrieveAttribute(WebRequest request, String attributeName) {
		...
		// 从session作用域获取属性值
		return request.getAttribute(storeAttributeName, WebRequest.SCOPE_SESSION/* 1 */);
	}
