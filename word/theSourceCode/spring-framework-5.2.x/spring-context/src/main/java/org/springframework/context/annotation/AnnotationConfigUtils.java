...
public abstract class AnnotationConfigUtils {
	 ...
	 * 内部管理的配置注释处理器的Bean名称。
	 * 真实的类是：ConfigurationClassPostProcessor
	 */
	...
	/**
	 * Register all relevant annotation post processors in the given registry. - 在给定的注册表中注册所有相关的注解后处理器。
	 ...
	 */
	public static Set<BeanDefinitionHolder> registerAnnotationConfigProcessors/* 注册注解配置处理器 */(
		...
		if (beanFactory != null) {
			...
			/* 1、设置依赖比较器：AnnotationAwareOrderComparator */

			if (!(beanFactory.getDependencyComparator/* 获取依赖比较器 */() instanceof AnnotationAwareOrderComparator)) {
				// 设置依赖比较器，也就是设置dependencyComparator属性，添加"AnnotationAwareOrderComparator"对象进去，主要是为了排序
				beanFactory.setDependencyComparator(AnnotationAwareOrderComparator.INSTANCE);
			}

			/* 2、设置自动装配解析器：ContextAnnotationAutowireCandidateResolver */

			if (!(beanFactory.getAutowireCandidateResolver() instanceof ContextAnnotationAutowireCandidateResolver)) {
				// 设置自动装配解析器
				// 设置autowireCandidateResolver属性(设置自动注入候选对象的解析器，用于判断beanDefinition是否为候选对象)—— 添加「ContextAnnotationAutowireCandidateResolver」类对象
				beanFactory.setAutowireCandidateResolver(new ContextAnnotationAutowireCandidateResolver());
			}
		}

		/**
		 * 下面的if很重要，需要理解注册每个bean的类型
		...
		/* 3、ConfigurationClassPostProcessor implements BeanDefinitionRegistryPostProcessor extends BeanFactoryPostProcessor */

		// 注册内部管理的用于处理@configuration的后置处理器的bean
		if (!registry.containsBeanDefinition(
			...
			// 注册ConfigurationClassPostProcessor bd到注册表当中
			beanDefs.add(registerPostProcessor(registry, def, CONFIGURATION_ANNOTATION_PROCESSOR_BEAN_NAME));
		}

		/* 4、AutowiredAnnotationBeanPostProcessor implements BeanPostProcessor */
		...
		/* 5、CommonAnnotationBeanPostProcessor implements BeanPostProcessor */

		// Check for JSR-250 support, and if present add the CommonAnnotationBeanPostProcessor. - 检查是否支持JSR-250，如果存在，请添加CommonAnnotationBeanPostProcessor。
		...
		/* 7、EventListenerMethodProcessor implements BeanFactoryPostProcessor */
		if (!registry.containsBeanDefinition(
		...
		8、DefaultEventListenerFactory：默认事件监听器工

		DefaultEventListenerFactory implements EventListenerFactory
	...
	private static BeanDefinitionHolder registerPostProcessor(
		 ...
		 * 		走的是GenericApplicationContext#registerBeanDefinition()
		 * 			GenericApplicationContext#registerBeanDefinition()内部调用了this.beanFactory.registerBeanDefinition()，也就是 DefaultListableBeanFactory
		 */
