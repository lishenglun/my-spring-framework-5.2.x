 ...
 */
public class DefaultContextLoadTimeWeaver/* 默认上下文加载期编织器 */ implements LoadTimeWeaver, BeanClassLoaderAware, DisposableBean {

	protected final Log logger = LogFactory.getLog(getClass());

	// InstrumentationLoadTimeWeaver
	// 题外：里面有Instrumentation实例：代表当前虚拟机的实例
	@Nullable
	...
	public void setBeanClassLoader(ClassLoader classLoader) {
		...
		// 检测当前虚拟机中的Instrumentation实例是否可用
		else if (InstrumentationLoadTimeWeaver.isInstrumentationAvailable()) {
			logger.debug("Found Spring's JVM agent for instrumentation");

			// ⚠️实例化InstrumentationLoadTimeWeaver
			// 题外：里面初始化了Instrumentation实例：代表当前虚拟机的实例
