...
public class ClassPathScanningCandidateComponentProvider implements EnvironmentCapable, ResourceLoaderAware {
	...
	private Set<BeanDefinition>  scanCandidateComponents(String basePackage) {
		Set<BeanDefinition> candidates = new LinkedHashSet<>();
		try {
			// 例如：classpath*:com/mashibing/selftag/**/*.class*
			String packageSearchPath = ResourcePatternResolver.CLASSPATH_ALL_URL_PREFIX/* classpath*: */ +
					resolveBasePackage(basePackage) + '/' + this.resourcePattern;
			// ASM读取指定包下所有的class文件对应的Resource
			// 有几个class文件，就能读取到几个Resource
			...
			for (Resource resource : resources) {
				...
				// 文件是否是可以读取的，只有是可以读取的，才能进行操作
				if (resource.isReadable()) {
					try {
						MetadataReader metadataReader/* 元数据读取器 */ = getMetadataReaderFactory().getMetadataReader(resource);
	...
	protected boolean isCandidateComponent(MetadataReader metadataReader) throws IOException {
		for (TypeFilter tf : this.excludeFilters/* 排除过滤器 */) {
			// 遍历额外的类，把不包含的给排除掉
			if (tf.match(metadataReader, getMetadataReaderFactory())) {
				return false;
			}
		}
		for (TypeFilter tf : this.includeFilters/* 包括过滤器 */) {
			if (tf.match(metadataReader, getMetadataReaderFactory())) {
