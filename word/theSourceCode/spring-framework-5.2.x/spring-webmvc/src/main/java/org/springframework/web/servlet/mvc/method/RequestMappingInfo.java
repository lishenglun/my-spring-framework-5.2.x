...
public final class RequestMappingInfo implements RequestCondition<RequestMappingInfo> {
	 ...
	 * mapping名称。
	 *
	 * 是@RequestMapping中的name属性值，如果没设置，则为null。
	 * 需要注意的是@RequestMapping中的默认属性是path，可不是name，所以我们在写@RequestMapping("/getUser")时，设置的是path属性值
	 */
	@Nullable
	 ...
	 *
	 * ⚠️注意：里面会包含请求路径
	 * (1)如果只有方法上有@RequestMapping，则是方法上@RequestMapping中配置的请求路径。
	 * (2)如果方法上有@RequestMapping，且类上也有@RequestMapping，则是【类上@RequestMapping中配置的请求路径 + # + 方法上@RequestMapping中配置的请求路径】
	 * (3)如果方法上有@RequestMapping，且类上也有@RequestMapping，还有前缀，则是【前缀 + 类上@RequestMapping中配置的请求路径 + # + 方法上@RequestMapping中配置的请求路径】
	 * (4)如果方法上有@RequestMapping，还有前缀，则是【前缀 + 方法上@RequestMapping中配置的请求路径】
	 * 具体逻辑参考：{@link RequestMappingHandlerMapping#getMappingForMethod(Method, Class)}
	 */
	...
	public RequestMappingInfo combine(RequestMappingInfo other) {
		// 【类上的@RequestMapping中设置的路径 + # + 方法的@RequestMapping中设置的路径】得到一个新的路径
	...
	private String combineNames(RequestMappingInfo other) {
		if (this.name != null && other.name != null) {
			...
			// 类上的@RequestMapping中设置的路径 + # + 方法的@RequestMapping中设置的路径
	 ...
	 *
	 * 检查此请求映射信息中的所有条件是否与提供的请求匹配，并返回一个潜在的新请求映射信息，其中包含针对当前请求量身定制的条件。
	 * <p>例如，返回的实例可能包含与当前请求匹配的 URL 模式的子集，并以最佳匹配模式在顶部进行排序。
	 *
	...
	public RequestMappingInfo getMatchingCondition(HttpServletRequest request) {
		...
		RequestMethodsRequestCondition/* 请求方法的条件 */ methods = this.methodsCondition.getMatchingCondition(request);
		if (methods == null) {
			return null;
		}
		ParamsRequestCondition/* 请求参数的条件 */ params = this.paramsCondition.getMatchingCondition(request);
		if (params == null) {
			return null;
		}
		HeadersRequestCondition/* 请求头的条件 */ headers = this.headersCondition.getMatchingCondition(request);
		if (headers == null) {
			return null;
		}
		ConsumesRequestCondition/* 可消费的Content-Type的条件 */ consumes = this.consumesCondition.getMatchingCondition(request);
		if (consumes == null) {
			return null;
		}
		ProducesRequestCondition/* 可生产的Content-Type的条件 */ produces = this.producesCondition.getMatchingCondition(request);
		if (produces == null) {
			return null;
		}
		PatternsRequestCondition/* 请求路径的条件 */ patterns = this.patternsCondition.getMatchingCondition(request);
		if (patterns == null) {
			return null;
		}
		RequestConditionHolder/* 自定义的条件 */ custom = this.customConditionHolder.getMatchingCondition(request);
		if (custom == null) {
			return null;
		}
		/*
		 * 创建匹配的RequestMappingInfo对象
		 * 为什么要创建RequestMappingInfo对象呢？
		 *
		 * 因为当前RequestMappingInfo对象，一个methodsCondition可以配置GET、POST、DELETE等等条件，
		 * 但是实际就匹配一个请求类型，此时methods只代表其匹配的那个。
		 */
	...
	private static class DefaultBuilder implements Builder {
		// 在根据@RequestMapping创建RequestMappingInfo时，保存了@RequestMapping中的路径信息
		...
		public RequestMappingInfo build() {
			...
			// 有通配符的路径，基本上是很具体的路径
			PatternsRequestCondition/* 模式请求条件 */ patternsCondition = ObjectUtils.isEmpty(this.paths) ? null :
					new PatternsRequestCondition(
							this.paths/* 请求路径 */, this.options.getUrlPathHelper(), this.options.getPathMatcher(),
							this.options.useSuffixPatternMatch(), this.options.useTrailingSlashMatch(),
							this.options.getFileExtensions());

			// 从配置操作中获取内容协商管理器
			ContentNegotiationManager/* 内容协商经理 */ manager = this.options.getContentNegotiationManager();
