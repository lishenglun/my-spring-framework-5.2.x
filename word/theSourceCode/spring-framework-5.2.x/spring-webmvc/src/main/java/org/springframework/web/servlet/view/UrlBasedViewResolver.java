...
public class UrlBasedViewResolver extends AbstractCachingViewResolver implements Ordered {
	...
	/** Map of static attributes, keyed by attribute name (String). —— 静态属性映射，以属性名称（字符串）为键。 */
	...
	protected View createView(String viewName, Locale locale) throws Exception {
		...
		// 上面翻译：如果此解析器不应该处理给定的视图，则返回null，以传递给链中的下一个解析器。

		/* 1、判断一下当前视图解析器，能够处理当前视图名称。不能处理，就返回null，让下一个视图解析器进行处理 */
		...
		/* 2、如果视图名称是以"redirect:"开头，则创建一个重定向视图对象RedirectView返回 */

		// Check for special "redirect:" prefix. —— 检查特殊的“重定向：”前缀。

		// 如果viewName是"redirect:"开头，则创建一个重定向视图对象RedirectView返回
		if (viewName.startsWith(REDIRECT_URL_PREFIX/* redirect: */)) {
			// 截取掉viewName中的"redirect:"重定向标识
			String redirectUrl = viewName.substring(REDIRECT_URL_PREFIX.length());

			// 重定向视图
			RedirectView view = new RedirectView(redirectUrl,
					isRedirectContextRelative/* 是重定向上下文相关的 */(), isRedirectHttp10Compatible/* 与重定向Http10兼容 */());
			...
			// 应用spring的生命周期方法，也就是调用initializeBean()，执行一些对应的逻辑
			return applyLifecycleMethods(REDIRECT_URL_PREFIX, view);
		}

		/* 3、如果视图名称是以"forward:"开头，则创建一个内部资源视图对象InternalResourceView返回 */

		// Check for special "forward:" prefix. —— 检查特殊的“转发:”前缀。

		// 如果viewName是"forward:"开头，则创建一个内部资源视图对象InternalResourceView返回
		...
		4、如果既不是重定向，也不是转发。则根据视图名称，创建一个普通的视图对象(JstlView)，进行返回。

		里面会根据我们web.xml中配置的前缀和后缀，与viewName进行拼接，得到一个真正的物理视图地址。后续就可以根据这个物理视图地址，读取页面进行返回。

		题外：viewName是一个逻辑视图名称

		 */

		// Else fall back to superclass implementation: calling loadView. —— 否则回退到超类实现：调用loadView()。
		// 创建视图名对应的View对象
	...
	protected boolean canHandle(String viewName, Locale locale) {
		// 获取当前视图处理器，能处理器的指定视图名称。
		// 一般情况下，viewNames为空，也就是代表没有限制，所有的视图名称都可以处理
		String[] viewNames = getViewNames();
		// 一般情况下，viewNames为空，满足viewNames == null，代表所有视图名都可以处理。
	...
	protected View loadView(String viewName, Locale locale) throws Exception {
		// 根据viewName创建View对象 —— JstlView
		// UrlBasedViewResolver#buildView()
		AbstractUrlBasedView/* 基于抽象URL的视图 */ view = buildView(viewName);
		// ⚠️应用bean生命周期的方法（spring在创建bean过程中的initializeBean()方法）来初始化view
		View result = applyLifecycleMethods(viewName, view);
		// AbstractUrlBasedView#checkResource()，恒定返回true，所以返回视图对象
	...
	protected AbstractUrlBasedView buildView(String viewName) throws Exception {
		...
		/**
		 * 例如：
		 * （1）我们在springmvc-01.xml中配置的ViewResolver：
		 * 	<bean id="viewResolver" class="org.springframework.web.servlet.view.InternalResourceViewResolver">
		 * 		<property name="prefix" value="/WEB-INF/jsp/"/>
		 * 		<property name="suffix" value=".jsp"/>
		 * 	</bean>
		 * （2）我们的请求路径是 http://127.0.0.1:8080/userlist
		 * （3）这里拼接前缀和后缀，得到的地址是：/WEB-INF/jsp/userlist.jsp
		 */
		// ⚠️拼接前缀和后缀
