...
public abstract class AbstractView extends WebApplicationObjectSupport implements View, BeanNameAware {
	...
	public void render(@Nullable Map<String, ?> model, HttpServletRequest request,
		...
		// 创建一个合并的Output输出的一个模型对象 —— 把我们对应的数据进行合并操作
		// 合并返回结果，将 Model 中的静态数据和请求中的动态数据进行合并
		Map<String, Object> mergedModel = createMergedOutputModel(model, request, response);
		// 准备响应（这里主要是IE浏览器里面单独的一些设置，修复IE浏览器中存在的BUG）
		prepareResponse(request, response);
		// ⚠️进行渲染InternalResourceView
		renderMergedOutputModel/* 渲染合并输出模型 */(mergedModel, getRequestToExpose(request), response);
	}
	...
	protected void exposeModelAsRequestAttributes(Map<String, Object> model,
		...
		model.forEach((name, value) -> {
			// 值不为空，就放入request作用域里面
			if (value != null) {
				request.setAttribute(name, value);
			}
			// 值为空，就将该属性名，从request作用域中移除
			else {
