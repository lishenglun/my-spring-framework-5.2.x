...
public abstract class AbstractCachingViewResolver extends WebApplicationObjectSupport implements ViewResolver {
	...
	// View 的缓存的映射
	private final Map<Object, View> viewAccessCache/* 视图访问缓存 */ = new ConcurrentHashMap<>(DEFAULT_CACHE_LIMIT);

	/** Map from view key to View instance, synchronized for View creation. */
	// View 的缓存的映射。相比 {@link #viewAccessCache} 来说，增加了 synchronized 锁
	@SuppressWarnings("serial")
	private final Map<Object, View> viewCreationCache/* 视图创建缓存 */ =
	...
	public View resolveViewName(String viewName, Locale locale) throws Exception {
		/* 1、如果禁用缓存，则创建viewName对应的View对象 */
		if (!isCache()) {
			// ⚠️创建viewName对应的View对象
			return createView(viewName, locale);
		}
		/* 2、如果不禁用缓存，则先从缓存中获取；如果获取不到，再创建viewName对应的View对象 */
		else {
			// 获得缓存KEY
			Object cacheKey = getCacheKey(viewName, locale);
			// 从viewAccessCache缓存中，获取View对象
			View view = this.viewAccessCache/* 视图访问缓存 */.get(cacheKey);
			// 如果获得不到缓存，则从viewCreationCache中，获取View对象
			if (view == null) {
				synchronized (this.viewCreationCache/* 视图创建缓存 */) {
					// 从viewCreationCache中，获取View对象
					view = this.viewCreationCache.get(cacheKey);
					if (view == null) {
						/**
						 * 1、InternalResourceViewResolver extends UrlBasedViewResolver
						 * 走的是UrlBasedViewResolver
						 * 通过里面的逻辑，里面的关键信息都有了，例如：里面会根据我们web.xml中配置的前缀和后缀，与viewName进行拼接，
						 * 得到一个真正的物理视图地址。后续就可以根据这个物理视图地址，读取页面进行返回
						 */
						// Ask the subclass to create the View object. —— 要求子类创建View对象
						// ⚠️创建viewName对应的View对象
						view = createView(viewName, locale);
						// 如果创建失败，但是cacheUnresolved为true ，则设置一个空的视图对象
						if (view == null && this.cacheUnresolved/* 缓存未解决 */) {
