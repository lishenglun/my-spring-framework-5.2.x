...
class CglibAopProxy implements AopProxy, Serializable {
	...
	/**
	 * 1、用来配置代理，实际advised = ProxyFactory
	 *
	 * 2、通过advised对ProxyFactory里面的属性进行修改工作
	 *
	 * 3、是在创建对应动态代理方式的时候，通过构造方法，赋予的值，参考：{@link DefaultAopProxyFactory#createAopProxy(AdvisedSupport)}
	 *
	 * The configuration used to configure this proxy. —— 用于配置此代理的配置。 */
	// ProxyFactory
	protected final AdvisedSupport/* 建议支持 */ advised;
	 ...
	 */
	public CglibAopProxy(AdvisedSupport config/* ProxyFactory */) throws AopConfigException {
	...
	public Object getProxy(@Nullable ClassLoader classLoader) {
		...
		try {
			// 获取目标类（当前要被代理对象的Class）
			Class<?> rootClass = this.advised.getTargetClass();
			Assert.state(rootClass != null, "Target class must be available for creating a CGLIB proxy"/* 目标类必须可用于创建CGLIB代理 */);

			Class<?> proxySuperClass = rootClass;
			// 判断目标类的名称中是否包含"$$"字符串，因为只有cglib生成的代理类才会包含"$$"字符串。
			// 如果包含了，则代表，当前目标类是cglib生成的类。如果目标类是cglib生成的类，则需要获取目标类所继承的父类，作为真实要被代理的类（因为这才是真实要被代理的类）
			// 题外：如果包含，也反应，"目标对象"是cglib生成的"代理对象"
			if (rootClass.getName().contains(ClassUtils.CGLIB_CLASS_SEPARATOR/* $$ */)) {
				// 目标类是cglib生成的类，所以获取目标类所继承的父类，作为真实要被代理的类
			...
			// Validate the class, writing log messages as necessary. —— 验证类，根据需要编写日志消息。
			/**
			 * 因为CGLIB是使用继承实现的，所以final、static的方法不能被代理增强
			 * 所以验证目标类中不能被代理增强的方法，打印出不能代理的方法名
			 */
			// 验证目标类中不能被代理增强的方法，打印出不能代理的方法名
			...
			/**
			 * 默认会添加额外的2个Spring接口：SpringProxy、Advised，后面的一些条件判断的时候会使用到
			 */
			// 获取完整的代理接口
			enhancer.setInterfaces(/* ⚠️ */AopProxyUtils.completeProxiedInterfaces(this.advised));
			// 修改cglib默认的代理类命名规则，标识这是由Spring产生的
			// 例如：cglib默认的是：com.MyCalculator$$EnhancerByCGLIB$$3ff25b35，改成了：com.MyCalculator$$BySpringCGLIB$$3ff25b35
			enhancer.setNamingPolicy/* 设置命名策略 */(SpringNamingPolicy.INSTANCE);
			enhancer.setStrategy(new ClassLoaderAwareGeneratorStrategy/* 类加载器感知生成器策略 */(classLoader));

			/* 获取所有的Callback(核心：DynamicAdvisedInterceptor) */
			// 题外：jdk的话，是InvocationHandler
			...
			// fixedInterceptorMap only populated at this point, after getCallbacks call above —— 在上面的getCallbacks()调用之后，fixedInterceptorMap仅在此时填充
			...
			// 创建代理对象
			return createProxyClassAndInstance(enhancer, callbacks)/* 返回一个代理对象 */;
		}
	...
	protected Object createProxyClassAndInstance(Enhancer enhancer, Callback[] callbacks) {
		enhancer.setInterceptDuringConstruction/* 在施工期间设置拦截 */(false);
		// 设置拦截器
		enhancer.setCallbacks(callbacks);
		/**
		 * 在enhancer.create()里面会生成代理类，以及创建代理对象
		 */
		return (this.constructorArgs != null && this.constructorArgTypes != null ?
				// 有参数值并且有参数类型，就采用有参构造器创建
	 ...
	 * validates it if not.
	 *
	 * 检查提供的 {@code Class} 是否已经过验证，如果没有则验证它。
	 */
	...
	private Callback[] getCallbacks(Class<?> rootClass) throws Exception {
		...
		/**
		 * expose-proxy属性的作用：
		 * 例如，某个代理类中包含m1和m2两个方法，都被通知增强了。当分别调用两个方法的时候，是会执行通知的！
		 * 但是，如果m1中this调用m2方法，那么在调用m2的时候，是不会执行通知的，如果想让m2也执行的话，就可以设置expose-proxy属性值为true！
		 */
		// 对于expose-proxy属性的处理，是否暴露当前对象为ThreadLocal模式，在当前上下文中能够进行引用
		boolean exposeProxy = this.advised.isExposeProxy();

		/**
		 * 在wrapIfNecessary() ——> createProxy() ——> proxyFactory.setFrozen(this.freezeProxy) 设置了
		 */
		// isFrozen作用：控制，ProxyFactory代理工厂被配置之后，是否还允许修改通知，默认值是false
		// 冰冻：当我这里添加完成之后，后续是否允许被修改：想不能被修改则设置为true，可以被修改就设置为false
		boolean isFrozen/* 冰冻 */ = this.advised.isFrozen();

		// 默认为true
		boolean isStatic = this.advised.getTargetSource().isStatic();

		/* 创建DynamicAdvisedInterceptor，里面保存了ProxyFactory，ProxyFactory里面有"能够增强当前bean的所有Advisor"，以及目标对象 */
		// Choose an "aop" interceptor (used for AOP calls). —— 选择一个“aop”拦截器（用于 AOP 调用）。
		// 将ProxyFactory封装在DynamicAdvisedInterceptor中，ProxyFactory里面有"能够增强当前bean的所有Advisor"
		Callback aopInterceptor = new DynamicAdvisedInterceptor/* 动态通知的拦截器 */(this.advised);

		// Choose a "straight to target" interceptor. (used for calls that are
		// unadvised but can return this). May be required to expose the proxy.
		// 上面的翻译：选择“直接瞄准”拦截器。（用于不建议但可以返回的调用）。可能需要公开代理。

		Callback targetInterceptor;
		if (exposeProxy) {
			targetInterceptor = (isStatic ?
					new StaticUnadvisedExposedInterceptor/* 静态的不通知的暴露拦截器 */(this.advised.getTargetSource().getTarget()) :
					new DynamicUnadvisedExposedInterceptor/* 动态的不通知的暴露拦截器 */(this.advised.getTargetSource()));
		}
		else {
			targetInterceptor = (isStatic ?
					new StaticUnadvisedInterceptor/* 静态的不通知的拦截器 */(this.advised.getTargetSource().getTarget()) :
					new DynamicUnadvisedInterceptor/* 动态的不通知的拦截器 */(this.advised.getTargetSource()));
		}

		// Choose a "direct to target" dispatcher (used for
		// unadvised calls to static targets that cannot return this).
		// 上面的翻译：选择“直接到目标”调度程序（用于对无法返回 this 的静态目标的不建议调用）。

		Callback targetDispatcher/* 目标调度员 */ = (isStatic ?
				new StaticDispatcher/* 静态调度器 */(this.advised.getTargetSource().getTarget()) : new SerializableNoOp/* 可序列化无操作 */());
		...
		// 上面的翻译：如果目标是静态的并且通知链被冻结，那么我们可以通过使用该方法的固定链将 AOP 调用直接发送到目标来进行一些优化。
		...
		if (isStatic && isFrozen) {
			...
			// 上面的翻译：TODO：这里的小内存优化（可以跳过没有建议的方法的创建）
			...
			// 上面的翻译：现在将 mainCallbacks 和 fixedCallbacks 的回调复制到回调数组中。
	...
	private static class DynamicAdvisedInterceptor implements MethodInterceptor, Serializable {
		...
		// ProxyFactory
		// 题外：ProxyFactory里面有"能够增强当前bean的所有Advisor"，以及目标对象
		private final AdvisedSupport advised;
		...
		public Object intercept(Object proxy, Method method, Object[] args, MethodProxy methodProxy) throws Throwable {
			 ...
			 * 1、TargetSource由来
			 *
			 * 在wrapIfNecessary() ——> createProxy(bean.getClass(), beanName, specificInterceptors, new SingletonTargetSource(bean))中，
			 * 的new SingletonTargetSource(bean)，其中的是bean是原生bean对象，也就是目标对象
			 *
			 * 2、SingletonTargetSource：里面存放了原生bean对象，也就是目标对象
			 */
			// SingletonTargetSource，里面包含了目标对象
			TargetSource targetSource = this.advised.getTargetSource();
			try {
				/**
				 * expose-proxy属性的作用：
				 * 例如，某个代理类中包含m1和m2两个方法，都被通知增强了。当分别调用两个方法的时候，是会执行通知的！
				 * 但是，如果m1中this调用m2的方法，那么在调用m2的时候，是不会执行通知的，如果想让m2也执行的话，就可以设置expose-proxy属性值为true！
				 *
				 * 简单概括：代理对象方法在调用代理对象方法的时候，是否执行拦截器链
				 */
				...
				/* 获取能够拦截当前方法的拦截器链 */
				// 先从缓存中获取，如果缓存中没有，则"从能够增强当前bean的Advisor中，获取能够增强当前方法的Advisor，然后从这些Advisor当作，获取其Advice对应的能够拦截当前方法的拦截器链"
				...
				/**
				 * 1、注意：
				 * 我之前的描述是：如果没有aop通知配置，则直接调用目标方法。
				 * 这是错误的描述，因为拦截器链中的拦截器可以不属于是aop通知的拦截器，例如：TransactionInterceptor
				 */
				/* 如果拦截器链为空，则直接调用目标方法 */
				if (chain.isEmpty() && Modifier.isPublic(method.getModifiers())) {
					...
					// 上面的翻译：我们可以跳过创建MethodInvocation：直接调用目标。请注意，最终调用者必须是InvokerInterceptor，因此我们知道它只对目标执行反射操作，并且没有热交换或花哨的代理。
				...
				/* 有拦截器，则执行方法的拦截器链 */
					 ...
					 */
					retVal = new CglibMethodInvocation(proxy, target, method, args, targetClass, chain/* 拦截器链 */, methodProxy).proceed();
				}
				// 验证返回值的类型
