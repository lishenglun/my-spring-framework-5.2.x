...
public abstract class AbstractAspectJAdvice implements Advice, AspectJPrecedenceInformation, Serializable {
	...
	public static JoinPoint currentJoinPoint() {
		...
		// JOIN_POINT_KEY的值为JoinPoint.class.getName()
	 ...
	 *
	 * 定义此通知的方面（参考bean）的名称（在确定通知优先级时使用，以便我们可以确定两条通知是否来自同一方面）。
	 */
	...
	/**
	 *
	 * returnValue：当执行后置返阿迪知的时候传值，其他为null
	 * Throwable：当执行后置异常旭知的时候传值，其他为null
	 *
	...
	protected JoinPointMatch getJoinPointMatch() {
		/**
		 * 这里从线程上下文当中获取MethodInvocation，看到这里，你也许会觉得奇怪，跟着文章分析来看，我们没有在设置过线程上下文的值啊
		 * 这里怎么获取到MethodInvocation的对象的呢？
		 ...
		 * 		// 如果获取到了Advisor则向Advisor集合中添加第一个元素，即ExposeInvocationInterceptor.ADVISOR
		 * 		// 也就是说，我们的Advisor列表中的第一个元素为ExposeInvocationInterceptor.ADVISOR，它是一个DefaultPointcutAdvisor的实例
		 * 		// 对于任何的目标方法都返回true，它的Advice是ExposeInvocationInterceptor
		 * 		advisors.add(0, ExposeInvocationInterceptor.ADVISOR);
		 * 		return true;
		 * }
		 * 这样我们就不难理解了，在调用ReflectiveMethodInvocation#proceed()的时候第一个调用的MethodInterceptor是ExposeInvocationInterceptor
		 * ExposeInvocationInterceptor的invoke()方法的内容如下：
		 * public Object invoke(MethodInvocation mi) throws Throwable{
		 *     // 先取出旧的MethodInvocation的值
		 *     MethodInvocation oldInvocation = invocation.get();
		 *     // 这里设置新的MethodInvocation就是这里了！！！
		 *     invocation.set(mi);
		 *     try{
		 *         return mi.proceed();
		 *     } finally {
