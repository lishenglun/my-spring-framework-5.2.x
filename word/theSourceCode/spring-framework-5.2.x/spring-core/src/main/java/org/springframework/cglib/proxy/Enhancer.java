...
public class Enhancer extends AbstractClassGenerator {
	...
	private Object createHelper() {
		// 检验callbackTypes、filter是否为空，以及为空时的处理
		preValidate();

		// ⚠️把参数对象都包含成了一个key对象
		// 通过newInstance()来创建EnhancerKey对象
		// 正常情况下，只需要new一个对象就可以调用方法了，但是KEY_FACTORY是一个Enhancer类型，是一个内部接口，需要动态代理来实现，最终是为了调用newInstance()
		Object key/* 代理对象 */ = KEY_FACTORY.newInstance(
				// 需要被代理的类的全限定类名
				...
				// 是否使用工厂
				useFactory,
				interceptDuringConstruction/* 施工期间拦截 */,
				// 序列化id
				serialVersionUID);
		// 设置当前enhancer的代理类的key标识
		this.currentKey = key;

		// 前面都是一些准备工作，这里正二八级的创建动态代理对象

		// 调用父类，即AbstractClassGenerator，创建代理类
		Object result/* 有可能是代理对象，也有可能是代理类Class对象 */ = super.create(key);
		return result;
	}
	...
	public void generateClass(ClassVisitor v) throws Exception {
		...
		// 检查final类无法被继承，finnal无法被继承，所以抛出异常
		if (TypeUtils.isFinal(sc.getModifiers()))
			throw new IllegalArgumentException("Cannot subclass final class " + sc.getName());
		// 找到该类所有声明了的构造函数
		List constructors = new ArrayList(Arrays.asList(sc.getDeclaredConstructors()));
		// 过滤调一些比如private之类的，不能被访问和继承的构造函数
		...
		// 声明所有必须为public的方法集合，这儿主要是代理接口的方法
