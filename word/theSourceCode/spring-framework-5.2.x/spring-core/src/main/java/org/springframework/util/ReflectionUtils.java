...
/**
 * 简单的实用程序，用于使用反射API，和处理反射异常
 *
 * Simple utility class for working with the reflection API and handling
 * reflection exceptions.
 *
 * 用于使用反射API，和处理反射异常的，简单实用程序类。
 *
 ...
 * @since 1.2.2
 */
public abstract class ReflectionUtils/* 反射工具类 */ {
	...
	public static final MethodFilter USER_DECLARED_METHODS =
			/**
			 * 1、method.isSynthetic()：判断方法是用户自定义的，还是由编译器生成的
			 * >>> 为了应对不同的jdk版本，编译器会往里面默认加入一些其它的方法，如果当前的方法是由编译器生成的，则isSynthetic()返回true
			 * >>> 如果是用户自定义的方法，则返回false
			 *
			 * 2、method.isBridge()：判断是不是桥接方法
			 * 我们用户自定义的方法都不是桥接方法
			 */
