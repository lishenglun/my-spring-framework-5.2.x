...
abstract public class AbstractClassGenerator<T> implements ClassGenerator {
	...
	protected static class ClassLoaderData {
		...
		public ClassLoaderData(ClassLoader classLoader) {
			...
			// 新建一个回调函数，这个回调函数的作用在于缓存中没获取到值时，调用传入的生成代理类并返回
			Function<AbstractClassGenerator, Object> load =
					new Function<AbstractClassGenerator, Object>() {
						public Object apply(AbstractClassGenerator gen) {
							// ⚠️这里面正式生成字节码文件
							// AbstractClassGenerator
							// spring在创建代理对象，gen = Enhancer
	...
	protected Object create(Object key) {
		try {
			...
			/**
			 * ClassLoaderData：可以理解我一个缓存对象
			 * 只不过此缓存对象中包含的是具体的业务逻辑处理过程，有两个function的函数式接口
			 * 一个是返回gen.key，对应的名称叫GET_KEY；
			 * 还有一个是为了创建具体的class,名字叫做load
			 */
			...
			// 设置一个全局key
			this.key = key;
			// 在刚创建的data(ClassLoaderData)中调用get方法，并将当前生成器，以及是否使用缓存的标识传入进去
			// 系统参数，System.getProperty("cglib.useCache", "true")返回的是生成好的代理类的class信息
			Object obj/* class对象 */ = data.get(this/* 生成器 */, getUseCache()/* 默认为true */);
