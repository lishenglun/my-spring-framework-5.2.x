...
public abstract class ClassUtils {
	...
	public static Class<?> forName(String name, @Nullable ClassLoader classLoader)
		...
		Class<?> clazz = resolvePrimitiveClassName/* 解析原始类名 */(name);
		// 如果clazz为null
		if (clazz == null) {
	...
	public static boolean isPrimitiveOrWrapper(Class<?> clazz) {
		Assert.notNull(clazz, "Class must not be null");
		return (clazz.isPrimitive()/* 是原始的 */ || isPrimitiveWrapper/* 是原始包装器 */(clazz));
	}
	 ...
	 *
	 * 假设通过反射设置，检查右侧类型是否可以分配给左侧类型。将原始包装类，视为可分配给相应的原始类型。
	 *
	...
	/**
	 * 获取当前类以及所有祖先类，实现的所有接口，然后把它作为一个set集合
	 *
	...
	public static Class<?> getUserClass(Class<?> clazz) {
		...
		1、判断当前类名称是否包含"cglib类分隔符" —— $$。
		如果包含，则表明可能是cglib生成的子类，于是就去获取父类；如果存在父类，并且父类不是Object，并返回父类Class对象

		 */
		// 判断当前类名称是否包含"cglib类分隔符" —— $$
		// 如果包含，则表明可能是cglib生成的子类，于是就去获取父类；如果存在父类，并且父类不是Object，并返回父类Class对象
		if (clazz.getName().contains(CGLIB_CLASS_SEPARATOR/* cglib类分隔符 */ /* $$ */ )) {
			// 获取父类
			Class<?> superclass = clazz.getSuperclass();
			// 如果存在父类，并且父类不是Object，就直接返回父类
			if (superclass != null && superclass != Object.class) {
				return superclass;
			}
		}

		/* 2、如果不包含、或者说包含，但是不存在父类、或者存在的父类是Object，则返回当前类Class对象 */
		return clazz;
	}
	...
	public static boolean hasConstructor(Class<?> clazz, Class<?>... paramTypes) {
		// 从clazz里面，获取paramTypes参数的构造器，如果存在，就返回true，不存在就返回false
	...
	public static Method getMostSpecificMethod(Method method, @Nullable Class<?> targetClass) {
		// 如果targetClass不为null，且targetClass不是声明method的类，且method在targetClass中可重写
		if (targetClass != null && targetClass != method.getDeclaringClass()/* 获取声明类 */ && isOverridable(method, targetClass)) {
			try {
				// 如果method是public方法
	...
	private static boolean isOverridable(Method method, @Nullable Class<?> targetClass) {
		...
		// 如果targetClass为null，或者获取声明method的类的包名等于目标类的包名是返回true，否则返回false
