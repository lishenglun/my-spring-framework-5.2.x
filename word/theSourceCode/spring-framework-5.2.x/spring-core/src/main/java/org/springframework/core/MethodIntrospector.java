...
public final class MethodIntrospector {
	...
	public static <T> Map<Method, T> selectMethods(Class<?> targetType, final MetadataLookup<T> metadataLookup) {
		...
		// 获取当前类以及所有祖先类，实现的所有接口，然后把它作为set集合给放到handlerTypes中
		...
		for (Class<?> currentHandlerType : handlerTypes) {
			...
			ReflectionUtils.doWithMethods(currentHandlerType, method -> {
				// 获取到具体的方法对象 —— 获取到你具体要执行的方法对象
				// 一般来说，你传进来的是哪个方法对象，获取到的还是那个方法对象
				Method specificMethod/* 具体方法 */ = ClassUtils.getMostSpecificMethod(method, targetClass);
				T result = metadataLookup.inspect(specificMethod);
				if (result != null) {
					// 查找桥接方法
					// 题外：我们用户自定义的方法都不是桥接方法！
	 ...
	 */
	@FunctionalInterface
	public interface MetadataLookup<T>/* 元数据查找 */ {
