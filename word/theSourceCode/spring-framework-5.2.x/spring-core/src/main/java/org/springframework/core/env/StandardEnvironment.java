...
public class StandardEnvironment extends AbstractEnvironment {
	...
	/**
	 *
	 * 在父类(AbstractEnvironment)的构造方法中，会调用此方法
	 *
	 ...
	 */
	@Override
	protected void customizePropertySources/* 自定义属性源 */(MutablePropertySources propertySources) {
		// propertySources是父类中的propertySources属性
		// propertySources = private final MutablePropertySources propertySources = new MutablePropertySources();

		propertySources.addLast(
				/* 系统属性值 */

				// 属性->属性源
				// systemEnvironment->属性源(map)
				new PropertiesPropertySource(SYSTEM_PROPERTIES_PROPERTY_SOURCE_NAME/* systemProperties 系统属性 */, getSystemProperties()));
		propertySources.addLast(
				/* 系统环境值 */

				// 系统环境属性源
				// systemProperties->属性源(map)
				new SystemEnvironmentPropertySource(SYSTEM_ENVIRONMENT_PROPERTY_SOURCE_NAME/* systemEnvironment 系统环境 */, getSystemEnvironment()));
	}
