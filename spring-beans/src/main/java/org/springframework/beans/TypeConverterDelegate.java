 ...
 */
class TypeConverterDelegate/* 类型转换委托器 */ {
	...
	public <T> T convertIfNecessary(@Nullable String propertyName, @Nullable Object oldValue, @Nullable Object newValue/* 参数值/属性值 */,
		...
		if (editor != null || (requiredType != null && !ClassUtils.isAssignableValue/* 是可赋值的 */(requiredType, convertedValue))) {
			...
			// ⚠️根据找到的自定义属性编辑器，进行️相关的处理工作 ———— 河北省_邯郸市_武安市 => Address{province='河北省', city='邯郸市', area='武安市'}
			// 使用editor将convertedValue转换为requiredType
			convertedValue = doConvertValue(oldValue, convertedValue, requiredType, editor/* 自定义的属性编辑器 */);
		}
		...
		if (requiredType != null) {
			/* 基本上这里面的一系列判断逻辑都不成立，都不会进行处理！而是直接返回转换后的值 */

			// Try to apply some standard type conversion rules if appropriate. —— 如果合适，尝试应用一些标准的类型转换规则。
			// 如果合适，尝试应用一些标准类型转换规则
			...
			if (!ClassUtils.isAssignableValue/* 是可赋值的 */(requiredType, convertedValue)) {
	...
	private Object doConvertValue(@Nullable Object oldValue, @Nullable Object newValue,
		...
		/* 2、参数类型不为空，参数类型不是数组，参数值是String数组 */
		...
		if (convertedValue instanceof String) {
			...
			if (editor != null) {
				...
				// ⚠️调用找到的自定义属性编辑器的setAsText()，进行️相关的处理工作
	 ...
	 */
	private Object doConvertTextValue(@Nullable Object oldValue, String newTextValue/* 参数值 */, PropertyEditor editor) {
		try {
			// 调用属性编辑器的setValue()方法。
			// PropertyEditor#setValue():设置属性的值，基本类型以包装类传入（自动装箱）.设置editor的属性值为oldValue
			...
			// Swallow and proceed. —— 吞下并继续。
		}
		// ⚠️调用属性编辑器的setAsText()方法，例如：AddressPropertyEditor#setAsText()
		// PropertyEditor.setAsText:用一个字符串去更新属性的内部值，这个字符串一般从外部属性编辑器传入；
		// 使用newTextValue更新内部属性值
		editor.setAsText(newTextValue);
		// 在AddressPropertyEditor#setAsText()中会调用setValue(address)，把最终的属性值进行保存到当前属性编辑器当中
		// 然后就可以editor.getValue()，获取最终的属性值进行返回！
		// 获取编辑后的属性值
