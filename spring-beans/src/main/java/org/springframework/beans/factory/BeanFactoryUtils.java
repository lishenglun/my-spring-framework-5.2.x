...
public abstract class BeanFactoryUtils {
	...
	public static String transformedBeanName(String name) {
		...
		// 截取掉所有&前缀，比如：&&&beanName，最终得到的是：beanName
		return transformedBeanNameCache.computeIfAbsent/* 计算如果不存在 */(name, beanName -> {
			do {
	...
	public static String[] beanNamesForTypeIncludingAncestors(
		...
		String[] result = lbf.getBeanNamesForType(type, includeNonSingletons, allowEagerInit);
		// 判断当前的bean工厂，是不是继承关系的bean工厂
		if (lbf instanceof HierarchicalBeanFactory) {
			HierarchicalBeanFactory hbf = (HierarchicalBeanFactory) lbf;
			// ListableBeanFactory：扩展BeanFactory使其支持迭代Ioc容器持有的Bean对象。
			// 注意如果ListableBeanFactory同时也是HierarchicalBeanFactory, 那么大多数情况下，
			// 只迭代当前Ioc容器持有的Bean对象，不会在体系结构中想父级递归迭代
			// 如果hbf的父工厂是ListableBeanFactory对象
			if (hbf.getParentBeanFactory() instanceof ListableBeanFactory) {
				/*
				递归操作，意味着，除了查找当前容器存在的type bd，还查找父容器里面存在的type bd
				 */
