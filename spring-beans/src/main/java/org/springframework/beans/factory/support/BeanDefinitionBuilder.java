...
public final class BeanDefinitionBuilder {
	 ...
	 * @since 5.0
	 */
	public static <T> BeanDefinitionBuilder genericBeanDefinition(Class<T> beanClass, Supplier<T> instanceSupplier/* 实例供应商 */) {
		BeanDefinitionBuilder builder = new BeanDefinitionBuilder(new GenericBeanDefinition());
		builder.beanDefinition.setBeanClass(beanClass);
	...
	/**
	 * Our current position with respect to constructor args. —— 我们目前关于构造函数参数的立场。
	 */
	private int constructorArgIndex;


	/**
	 * Enforce the use of factory methods. —— 强制使用工厂方法。
	 */
	 ...
	 * ⚠️以原始（未验证）形式返回当前 BeanDefinition 对象。
