...
class DisposableBeanAdapter implements DisposableBean, Runnable, Serializable {
	...
	public DisposableBeanAdapter(Object bean, String beanName, RootBeanDefinition beanDefinition,
		...
		if (destroyMethodName != null &&
				// this.invokeDisposableBean：bean是否是DisposableBean实例 && "destroy"没有受外部管理的销毁方法
				// destroyMethodName是'destroy'
				!(this.invokeDisposableBean && "destroy".equals(destroyMethodName)) &&
				// destroyMethodName不是外部受管理的销毁方法
				!beanDefinition.isExternallyManagedDestroyMethod(destroyMethodName)) {
			this.destroyMethodName = destroyMethodName;
			// 根据BeanDefinition是否允许访问非公共构造函数和方法的情况来查找最小参数(最好是name)的销毁方法对象
			...
			if (destroyMethod == null) {
				...
				// 参数类型数组为1 && 第一个参数类型不是Boolean类型
				else if (paramTypes.length == 1 && boolean.class != paramTypes[0]) {
					throw new BeanDefinitionValidationException("Method '" + destroyMethodName + "' of bean '" +
							beanName + "' has a non-boolean parameter - not supported as destroy method");
				}
				// 获取destroyMethod想要的接口方法对象，如果找不到，则返回原始方法
	...
	public static boolean hasApplicableProcessors(Object bean, List<BeanPostProcessor> postProcessors) {
		if (!CollectionUtils.isEmpty(postProcessors)) {
			for (BeanPostProcessor processor : postProcessors) {
				if (processor instanceof DestructionAwareBeanPostProcessor) {
					DestructionAwareBeanPostProcessor dabpp = (DestructionAwareBeanPostProcessor) processor;
					// requiresDestruction()：bean是否需要被毁灭。默认返回为true，代表需要要被毁灭；为false的话，代表不需要
					if (dabpp.requiresDestruction(bean)) {
						return true;
					}
				}
