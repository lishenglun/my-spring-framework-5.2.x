...
public class PathMatchingResourcePatternResolver implements ResourcePatternResolver {
	...
	/**
	 * @param locationPattern 例子：spring-06-config.xml
	 * @return
	 * @throws IOException
	 */
	@Override
	public Resource[] getResources(String locationPattern) throws IOException {
		...
		if (locationPattern.startsWith(CLASSPATH_ALL_URL_PREFIX/* classpath*: */)) {
			// a class path resource (multiple resources for same name possible) —— 一个类路径资源（可能有多个同名资源）
			if (getPathMatcher().isPattern(locationPattern.substring(CLASSPATH_ALL_URL_PREFIX.length()/* classpath*: */))) {
			...
			if (getPathMatcher().isPattern(locationPattern.substring(prefixEnd))) {
				...
				// resource：只是包装了一下"locationPattern=spring-06-config.xml"和"ClassLoader"，并没有实际加载spring-06-config.xml文件信息
				Resource resource = getResourceLoader().getResource(locationPattern);
				// 将resource放入Resource[]中
