...
/**
 * 参考：https://my.oschina.net/u/3039151/blog/3136645
 *
...
public abstract class AnnotatedElementUtils {
	 ...
	 * {@linkplain AnnotatedElementUtils class-level javadoc}.
	 *
	 * 确定指定 {@code annotationType} 的注解是否在提供的 {@link AnnotatedElement} 上或在指定元素的注解层次结构 <em>above<em> 内 <em>available<em>。
	...
	public static boolean hasAnnotation(AnnotatedElement element, Class<? extends Annotation> annotationType) {
		// Shortcut: directly present on the element, with no merging needed? —— 快捷方式：直接出现在元素上，不需要合并？
		if (AnnotationFilter.PLAIN.matches(annotationType) ||
				AnnotationsScanner.hasPlainJavaAnnotationsOnly(element)) {
			return element.isAnnotationPresent(annotationType);
		}
		// Exhaustive retrieval of merged annotations... —— 合并注解的详尽检索...
	...
	/**
	 * 在element上查询annotationType类型的注解，然后将查询出的多个annotationType类型注解属性合并到查询的第一个注解中，
	 * 也就是多个相同注解的合并！
	 *
	 * Find the first annotation of the specified {@code annotationType} within
	 * the annotation hierarchy <em>above</em> the supplied {@code element},
	 ...
	 */
	@Nullable
	public static <A extends Annotation> A findMergedAnnotation/* 找到多个相同的注解并合并 */(AnnotatedElement element, Class<A> annotationType) {
		// Shortcut: directly present on the element, with no merging needed? —— 快捷方式：直接出现在元素上，不需要合并？
		if (AnnotationFilter.PLAIN.matches(annotationType) ||
				AnnotationsScanner.hasPlainJavaAnnotationsOnly/* 仅具有纯Java注解 */(element)) {
			return element.getDeclaredAnnotation(annotationType);
		}
		// Exhaustive retrieval of merged annotations... —— 合并注释的详尽检索...
