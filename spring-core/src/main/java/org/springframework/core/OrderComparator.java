...
public class OrderComparator implements Comparator<Object> {
	...
	private int doCompare(@Nullable Object o1, @Nullable Object o2, @Nullable OrderSourceProvider sourceProvider) {
		...
		// 1、如果o1实现了PriorityOrdered接口，o2没有，则o1排前面
		if (p1 && !p2) {
			return -1;
		}
		...
		// 3、如果o1和o2都实现或者都没有实现PriorityOrdered接口

		// 拿到o1的order值，如果没有实现Ordered接口，值为Ordered.LOWEST_PRECEDENCE（💡Ordered.LOWEST_PRECEDENCE = int的最大值）
		int i1 = getOrder(o1, sourceProvider);
		// 拿到o2的order值，如果没有实现Ordered接口，值为Ordered.LOWEST_PRECEDENCE
		int i2 = getOrder(o2, sourceProvider);
		// 通过order值排序(order值越小，优先级越高)
		// 如果i1和i2两个都是int的最大值，则意味着两个类都没实现。既然两个类都没实现，那么它的顺序就不重要了
	...
	private int getOrder(@Nullable Object obj, @Nullable OrderSourceProvider sourceProvider) {
		...
		// ⚠️
		return (order != null ? order : getOrder(obj)/* ⚠️ */);
	}
	...
	protected int getOrder(@Nullable Object obj) {
		...
		// ⚠️返回Ordered.LOWEST_PRECEDENCE
		// Ordered.LOWEST_PRECEDENCE = int的最大值
