...
public class AnnotationTransactionAttributeSource extends AbstractFallbackTransactionAttributeSource
	 ...
	 * 其中的值被加人了SpringTransactionAnnotationParser，也就是当进行属性获取的时候其实是使用SpringTransactionAnnotationParser类的parseTransactionAnnotation()方法进行解析的。
	 * SpringTransactionAnnotationParser里面是寻找和解析@Transaction
	 */
	// 事务注解解析器，例如：SpringTransactionAnnotationParser用户寻找和解析@Transaction
	...
	/**
	 * 遍历TransactionAnnotationParser(事务注解解析器)，用事务注解解析器，寻找和解析对应的事务注解，构建TransactionAttribute。
	 * 典型的就是：SpringTransactionAnnotationParser：寻找和解析@Transactional信息，构建一个TransactionAttribute返回
	 *
	 * 题外：如果有一个事务注解解析器解析得到了TransactionAttribute，那么就不会再走下一个事务注解解析器，也就是说：只会采用一个事务注解解析器的结果，实际上也就是采用了一个事务注解解析器
	 *
	...
	protected TransactionAttribute determineTransactionAttribute(AnnotatedElement element) {
		// 遍历事务注解解析器
		for (TransactionAnnotationParser/* 事务注解解析器 */ parser : this.annotationParsers) {
			// 用事务注解解析器，寻找和解析element中的事务注解，得到一个TransactionAttribute
			// 典型的：parser = ⚠️SpringTransactionAnnotationParser，用来寻找和解析@Transactional信息，构建一个TransactionAttribute返回
			// 题外：从这里也可以得知，如果有一个事务注解解析器解析得到了TransactionAttribute，那么就不会再走下一个事务注解解析器，也就是说：只会采用一个事务注解解析器的结果，实际上也就是采用了一个事务注解解析器
