...
public abstract class AbstractTransactionStatus implements TransactionStatus {
	...
	/**
	 * Create a savepoint and hold it for the transaction. —— 创建一个保存点并为事务保存它。
	 * @throws org.springframework.transaction.NestedTransactionNotSupportedException
	 * if the underlying transaction does not support savepoints
	 */
	public void createAndHoldSavepoint() throws TransactionException {
		// getSavepointManager()：获取保存点管理器，但是返回的其实是DataSourceTransactionObject
		// createSavepoint()：创建一个保存点！
	...
	public void rollbackToHeldSavepoint() throws TransactionException {
		...
		/**
		 * 1、getSavepointManager()：
		 * （1）这里使用的是JDBC的方式进行数据库连接，那么getSavepointManager()函数返回的是JdbcTransactionObjectSupport，
		 */
		// 回滚到保存点；并重置回滚标记，表示不需要回滚了
		getSavepointManager().rollbackToSavepoint(savepoint);

		// 释放保存点
		getSavepointManager().releaseSavepoint(savepoint);

		// 设置保存点为null，也就是说，保存点已经不存在了！
		setSavepoint(null);
	}
