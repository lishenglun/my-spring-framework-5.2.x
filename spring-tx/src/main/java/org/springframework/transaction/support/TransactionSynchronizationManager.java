...
public abstract class TransactionSynchronizationManager/* 事务同步管理器 */ {
	...
	// 当前事务的TransactionSynchronization
	...
	// 当前事务"是否是实际活跃的事务的标识"（事务是否是激活状态的标识），用于判断当前事务是否已经激活
	...
	/**
	 * 从本地线程变量中获取dataSource对应的连接持有器
	 *
	 * 通过连接器，可以获取当前线程中已经存在的连接，保证线程中的数据库操作都是使用同一个事务连接
	 *
	 * Retrieve a resource for the given key that is bound to the current thread. —— 检索绑定到当前线程的给定键的资源。
	 * @param key the key to check (usually the resource factory)
	 ...
	 */
	@Nullable
	public static Object getResource(Object key/* dataSource */) {
		// 必要时解包装datasource，一般不需要解包，都是原路返回，传进来什么datasource，返回什么datasource
		// 题外：unwrap：解包装
		Object actualKey/* DataSource */ = TransactionSynchronizationUtils.unwrapResourceIfNecessary/* 如果有需要，解除资源的包装 */(key);

		// 从resources ThreadLoad中获取数据源对应的连接持有器
		// 题外：第一次进来为null
		Object value = doGetResource(actualKey);
	...
	private static Object doGetResource(Object actualKey) {
		...
		// 从map里面获取当前数据源对应的连接持有器
		Object value = map.get(actualKey);

		// Transparently remove ResourceHolder that was marked as void... —— 透明地删除标记为无效的 ResourceHolder...
	...
	/**
	 * 往resources ThreadLoad中，存入数据源和对应的连接持有器
	 *
	...
	public static void bindResource(Object key, Object value) throws IllegalStateException {
		...
		Map<Object, Object> map = resources.get();
		// set ThreadLocal Map if none found —— 如果没有找到，则设置ThreadLocal Map
		if (map == null) {
			map = new HashMap<>();
			resources.set(map);
		}
		Object oldValue = map.put(actualKey/* 数据源 */ , value/* 连接持有器 */);
		// Transparently suppress a ResourceHolder that was marked as void... —— 透明地抑制标记为无效的ResourceHolder...
	...
	public static Object unbindResource(Object key) throws IllegalStateException {
		// 数据源
		Object actualKey = TransactionSynchronizationUtils.unwrapResourceIfNecessary/* 必要时解包资源 */(key);
		// ⚠️
	...
	private static Object doUnbindResource(Object actualKey) {
		...
		// 移除当前线程中，对应数据源对应的连接持有器，并返回数据源对应的连接持有器
		Object value = map.remove(actualKey);
		// Remove entire ThreadLocal if empty... —— 如果为空，则删除整个 ThreadLocal...
		if (map.isEmpty()) {
			// 清空整个resources ThreadLocal
			resources.remove();
		}
	...
	public static boolean isSynchronizationActive() {
		// 当前线程里面的事务同步信息不为null，代表当前之前这个事务已经同步过了，所以返回true
		return (synchronizations.get() != null);
	}

	/**
	 * 初始化synchronizations ThreadLocal中TransactionSynchronization集合
	 *
	...
	public static List<TransactionSynchronization> getSynchronizations() throws IllegalStateException {
		...
		// 返回不可修改的快照，以避免在迭代和调用同步回调时发生 ConcurrentModificationExceptions，而同步回调又可能会注册进一步的同步。
		if (synchs.isEmpty()) {
