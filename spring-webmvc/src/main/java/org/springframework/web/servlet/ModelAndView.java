...
public class ModelAndView {
	...
	// 有肯能是一个String类型的视图名称，也有可能是一个View对象
	@Nullable
	private Object view;
	...
	public ModelAndView(@Nullable String viewName, @Nullable Map<String, ?> model, @Nullable HttpStatus status) {
		...
		if (model != null) {
			/**
			 * getModelMap()：为当前的ModelAndView对象，创建一个新的ModelMap对象
			 * addAllAttributes(model)：把ModelAndViewContainer中的model属性全部加入到这个新的ModelMap对象中
			 */
			// 为当前的ModelAndView对象，创建一个新的ModelMap对象，并且把ModelAndViewContainer中的model属性全部加入到这个新的ModelMap对象中
			getModelMap().addAllAttributes(model);
		}
		// 状态
		this.status = status;
	...
	public boolean wasCleared() {
		// 当前ModelAndView已经被清理 && (view为null，并且model也是空)
