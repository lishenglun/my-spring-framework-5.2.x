...
public abstract class WebContentGenerator extends WebApplicationObjectSupport {
	...
	protected final void checkRequest(HttpServletRequest request) throws ServletException {
		/* 1、通过supportedMethods支持的请求方法集合，判断是否支持当前的请求方法。如果包含，也就是不支持，则抛出异常 */
		// Check whether we should support the request method. —— 检查我们是否应该支持当前请求方法。
		String method = request.getMethod();
		if (this.supportedMethods/* 支持的请求方法 */ != null && !this.supportedMethods.contains(method)) {
			throw new HttpRequestMethodNotSupportedException(method, this.supportedMethods);
		}

		/* 2、如果session是必须存在，则判断session是否实际存在。如果不存在session，就抛出异常 */
		// Check whether a session is required. —— 检查是否需要会话。
		if (this.requireSession/* 是否必须要存在一个session对象，默认为false */ && request.getSession(false) == null) {
			// 需要预先存在的会话(session)，但未找到
