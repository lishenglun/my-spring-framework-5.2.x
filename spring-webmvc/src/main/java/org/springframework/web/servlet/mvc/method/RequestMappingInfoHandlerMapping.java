...
public abstract class RequestMappingInfoHandlerMapping extends AbstractHandlerMethodMapping<RequestMappingInfo> {
	...
	protected Set<String> getMappingPathPatterns(RequestMappingInfo info) {
		// 通过PatternsRequestCondition，获取请求路径
		return info.getPatternsCondition().getPatterns()/* 获取请求路径 */;
	}
	...
	protected HandlerMethod getHandlerInternal(HttpServletRequest request) throws Exception {
		// ⚠️从request中移除掉"可生产的媒体类型"属性值
		// 🤔️为什么要移除？
		request.removeAttribute(PRODUCIBLE_MEDIA_TYPES_ATTRIBUTE/* org.springframework.web.servlet.HandlerMapping.producibleMediaTypes */);
		try {
			/*

			1、️通过请求路径获取HandlerMethod，作为handler对象。
			...
			ProducesRequestCondition.clearMediaTypesAttribute/* 清除媒体类型属性 */(request);
		}
	}
	...
	protected void handleMatch(RequestMappingInfo info, String lookupPath, HttpServletRequest request) {
		...
		// 设置"uri模板变量"到request中
		Map<String, String> decodedUriVariables = getUrlPathHelper().decodePathVariables(request, uriVariables);
		request.setAttribute(HandlerMapping.URI_TEMPLATE_VARIABLES_ATTRIBUTE, decodedUriVariables);

		// 设置"可生成的媒体类型"到request中
		if (!info.getProducesCondition().getProducibleMediaTypes().isEmpty()) {
			Set<MediaType> mediaTypes = info.getProducesCondition().getProducibleMediaTypes();
			request.setAttribute(PRODUCIBLE_MEDIA_TYPES_ATTRIBUTE/* org.springframework.web.servlet.HandlerMapping.producibleMediaTypes */, mediaTypes);
		}
	}
