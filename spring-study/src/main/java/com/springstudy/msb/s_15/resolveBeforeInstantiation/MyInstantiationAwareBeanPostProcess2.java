 ...
 */
public class MyInstantiationAwareBeanPostProcess2 implements InstantiationAwareBeanPostProcessor {


	@Override
	public Object postProcessBeforeInstantiation(Class<?> beanClass, String beanName) throws BeansException {
		// 1、可以在这里面实现动态代理的代码，返回一个动态代理的对象，来代替要实例化的bean，就采用这个bean，后续不会再实例化bean对象了
		// 2、也可以返回一个普通的对象，如果返回普通的对象，那么方法调用的时候就不会有前置处理逻辑了，就不会被拦截了
		System.out.println("beanName：" + beanName + "--------执行，实例化之前的，方法");
		if (beanClass == BeforeInstantiation2.class) {
